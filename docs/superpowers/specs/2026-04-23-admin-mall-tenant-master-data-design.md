...
5. Create one professional terminal master-data workbench under
   `2-ui/2.2-business`.
6. Route the primary display to the master-data workbench after terminal
   activation while leaving secondary display behavior unchanged.
...
| `terminal-auth-service` | Reserved HTTP boundary for future terminal login/logout/session APIs |
...
3. Validate projection envelope `schema_version`.
...
`2-ui/2.2-business/master-data-workbench`

Published package:

`@impos2/ui-business-master-data-workbench`

Module name:

`ui.business.master-data-workbench`
...
Activation routing changes only the primary display success target.

| Display | Before activation | After activation |
| --- | --- | --- |
| Primary | Existing activation screen | Master-data workbench |
| Secondary | Existing secondary activation screen | Existing secondary welcome screen |
...
3. The master-data workbench screen is registered by the
   `ui-business-master-data-workbench` package.
...
Admin edits product in mock-admin-mall-tenant-console
  ...
  -> primary display updates immediately
```
...
5. Retail-shell tests:
   ...
   - After activation, primary switches to master-data workbench.
   - Secondary activation/success behavior remains unchanged.
6. Live integration:
...
2. The existing `mock-terminal-platform` may need no protocol change if its
   batch-upsert API is sufficient.
