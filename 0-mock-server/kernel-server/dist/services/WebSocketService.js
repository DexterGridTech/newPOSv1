...
export class WebSocketService {
    wss = null;
    connections = new Map(); // 设备连接
    webClients = new Set(); // Web管理后台连接
     ...
     * 处理Web管理后台连接
     */
    handleWebClientConnection(ws, req) {
        try {
            const clientIp = req.socket.remoteAddress;
     ...
     * 推送单元数据变更
     */
    async pushUnitDataChange(deviceId, group, updated, deleted) {
     ...
     * 推送远程指令
     */
    async pushRemoteCommand(deviceId, command) {
     ...
     * 广播设备在线状态到所有Web客户端
     */
    broadcastDeviceOnlineStatus(deviceId, online) {
