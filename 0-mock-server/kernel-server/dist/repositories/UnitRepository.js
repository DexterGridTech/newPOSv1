...
export class UnitRepository {
    ...
    `);
        stmt.run(id, data.name, data.key, data.type, data.parentId || null, JSON.stringify(rootPath), data.entityUnitId || null, data.modelUnitId || null, data.activeCode || null, data.deactiveCode || null, now, now);
     ...
     * 根据激活码查找终端
     */
    findByActiveCode(activeCode) {
        const db = getDatabase();
        const stmt = db.prepare('SELECT * FROM unit WHERE active_code = ? AND type = ?');
        const row = stmt.get(activeCode, UnitType.TERMINAL);
        return row ? this.mapToUnit(row) : null;
    }
    /**
     * 查找所有单元
