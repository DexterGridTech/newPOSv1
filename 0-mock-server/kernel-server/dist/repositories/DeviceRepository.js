...
export class DeviceRepository {
    ...
    `);
        stmt.run(deviceInfo.id, deviceInfo.manufacturer, deviceInfo.os, deviceInfo.osVersion, deviceInfo.cpu, deviceInfo.memory, deviceInfo.disk, deviceInfo.network, terminalId, token, null, now, now);
     ...
     */
    findByEntityInRootPath(entityId) {
        const db = getDatabase();
        const stmt = db.prepare(`
      SELECT d.* FROM device d
     ...
     * 更新设备的操作实体
     */
    updateOperatingEntity(id, operatingEntityId) {
        const db = getDatabase();
     ...
     * 记录设备连接
     */
    createConnection(deviceId, clientIp, userAgent) {
        const db = getDatabase();
        const id = generateId();
        const now = Date.now();
        const stmt = db.prepare(`
    ...
    `);
        stmt.run(id, deviceId, now, null, clientIp || null, userAgent || null, DeviceConnectionStatus.CONNECTED);
     ...
     * 查找设备的所有连接记录
     */
    findConnectionsByDeviceId(deviceId) {
        const db = getDatabase();
     ...
     * 查找设备的当前连接状态
     */
    findCurrentConnection(deviceId) {
        const db = getDatabase();
        const stmt = db.prepare(`
      ...
      LIMIT 1
    `);
        const row = stmt.get(deviceId, DeviceConnectionStatus.CONNECTED);
        return row ? this.mapToConnectionInfo(row) : null;
    }
    /**
     * 检查设备是否在线
