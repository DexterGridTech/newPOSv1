...
export class CommandRepository {
    ...
    `);
        stmt.run(id, data.name, data.type, data.valid !== undefined ? (data.valid ? 1 : 0) : 1, data.defaultPayload || null, now, now);
    ...
    createCommand(itemId, data) {
      ...
      VALUES (?, ?, ?, ?, ?, ?)
    `);
        stmt.run(id, item.type, data.payload || item.defaultPayload || null, data.requestId || null, data.sessionId || null, now);
    ...
    `);
        stmt.run(id, commandId, deviceId, command.type, command.requestId, command.sessionId, now, sendResult ? 1 : 0, null, null);
     ...
     * 根据commandId查找指令记录
     */
    findRecordByCommandId(commandId) {
        const db = getDatabase();
     ...
     * 更新指令接收结果
     */
    updateRecordReceiveResult(commandId, receiveResult) {
        const db = getDatabase();
        const now = Date.now();
        const stmt = db.prepare(`
