...
function initDefaultData(database) {
    ...
    const existingCommand = database.prepare('SELECT id FROM command_item WHERE type = ?').get('kernel.core.terminal.sendStateToServer');
    if (!existingCommand) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `).run('sendStateToServer', '获取设备状态', 'kernel.core.terminal.sendStateToServer', 1, '{}', now, now);
