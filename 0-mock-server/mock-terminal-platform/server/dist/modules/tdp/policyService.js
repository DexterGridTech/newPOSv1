...
const getTargetTerminalIdsForPolicy = (input) => {
    ...
    const rows = db.select().from(selectorGroupMembershipsTable).where(and(eq(selectorGroupMembershipsTable.sandboxId, input.sandboxId), eq(selectorGroupMembershipsTable.groupId, input.scopeKey))).all();
...
export const createProjectionPolicy = (input) => {
    ...
    const existing = db.select().from(projectionPoliciesTable).where(and(eq(projectionPoliciesTable.sandboxId, sandboxId), eq(projectionPoliciesTable.topicKey, input.topicKey), eq(projectionPoliciesTable.itemKey, input.itemKey), eq(projectionPoliciesTable.scopeType, input.scopeType), eq(projectionPoliciesTable.scopeKey, input.scopeKey))).get();
    if (existing?.enabled) {
    ...
    if (existing) {
        db.update(projectionPoliciesTable).set({
            ...
            updatedAt: timestamp,
        }).where(and(eq(projectionPoliciesTable.sandboxId, sandboxId), eq(projectionPoliciesTable.policyId, policyId))).run();
    }
    else {
...
const getProjectionPolicyRecord = (sandboxId, policyId) => {
    const policy = db.select().from(projectionPoliciesTable).where(and(eq(projectionPoliciesTable.sandboxId, sandboxId), eq(projectionPoliciesTable.policyId, policyId))).get();
    if (!policy) {
...
const listHistoricalTargetTerminalIds = (input) => {
  ...
  `).all(input.sandboxId, input.topicKey, input.scopeType, input.scopeKey, input.itemKey);
...
export const listProjectionPolicies = (sandboxId) => {
    assertSandboxUsable(sandboxId);
...
export const materializeEnabledGroupPoliciesForTerminal = (input) => {
    ...
    const policies = db.select().from(projectionPoliciesTable)
        .where(and(eq(projectionPoliciesTable.sandboxId, sandboxId), eq(projectionPoliciesTable.scopeType, 'GROUP'), eq(projectionPoliciesTable.enabled, 1)))
        .all()
    ...
    const items = policies.map(policy => {
        const projection = db.select().from(projectionsTable).where(and(eq(projectionsTable.sandboxId, sandboxId), eq(projectionsTable.topicKey, policy.topicKey), eq(projectionsTable.scopeType, policy.scopeType), eq(projectionsTable.scopeKey, policy.scopeKey), eq(projectionsTable.itemKey, policy.itemKey))).get();
        if (!projection) {
            return upsertProjectionBatch({
                sandboxId,
                projections: [{
                        ...
                        payload: parseJson(policy.payloadJson, {}),
        ...
        return fanoutExistingProjectionToTerminalIds({
            ...
            payload: parseJson(projection.payloadJson, {}),
            targetTerminalIds: [input.terminalId],
        });
    });
    return {
...
export const updateProjectionPolicy = (input) => {
    ...
    db.update(projectionPoliciesTable)
        ...
        .where(and(eq(projectionPoliciesTable.sandboxId, sandboxId), eq(projectionPoliciesTable.policyId, input.policyId)))
        .run();
