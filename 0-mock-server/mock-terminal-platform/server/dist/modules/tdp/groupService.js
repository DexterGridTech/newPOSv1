...
import { projectionPoliciesTable, projectionsTable, selectorGroupMembershipsTable, selectorGroupsTable, terminalsTable, } from '../../database/schema.js';
...
const parseCapabilities = (deviceInfo) => {
    const capabilities = deviceInfo.capabilities;
    if (!Array.isArray(capabilities))
        return [];
    return capabilities.filter(item => typeof item === 'string');
};
export const getTerminalRuntimeFacts = (sandboxId, terminalId) => {
    const terminal = db.select().from(terminalsTable).where(and(eq(terminalsTable.sandboxId, sandboxId), eq(terminalsTable.terminalId, terminalId))).get();
    if (!terminal) {
        throw new Error('终端不存在');
    }
    const deviceInfo = parseJson(terminal.deviceInfoJson, {});
...
const getEnabledSelectorGroups = (sandboxId) => db.select().from(selectorGroupsTable)
...
export const createSelectorGroup = (input) => {
    ...
    const existing = db.select().from(selectorGroupsTable).where(and(eq(selectorGroupsTable.sandboxId, sandboxId), eq(selectorGroupsTable.groupCode, groupCode))).get();
    if (existing) {
...
const mapSelectorGroup = (item) => ({
    ...item,
    enabled: Boolean(item.enabled),
    selectorDslJson: parseJson(item.selectorDslJson, {}),
});
export const listSelectorGroups = (sandboxId) => {
    assertSandboxUsable(sandboxId);
...
export const updateSelectorGroup = (input) => {
    const sandboxId = input.sandboxId;
    assertSandboxUsable(sandboxId);
    const current = db.select().from(selectorGroupsTable).where(and(eq(selectorGroupsTable.sandboxId, sandboxId), eq(selectorGroupsTable.groupId, input.groupId))).get();
    if (!current) {
    ...
    const duplicated = db.select().from(selectorGroupsTable).where(and(eq(selectorGroupsTable.sandboxId, sandboxId), eq(selectorGroupsTable.groupCode, nextGroupCode))).get();
    if (duplicated && duplicated.groupId !== input.groupId) {
        throw new Error('groupCode 已存在');
    }
    const timestamp = now();
    const nextSelector = input.selectorDslJson ?? parseJson(current.selectorDslJson, {});
    ...
    const updated = db.select().from(selectorGroupsTable).where(and(eq(selectorGroupsTable.sandboxId, sandboxId), eq(selectorGroupsTable.groupId, input.groupId))).get();
    return {
...
export const recomputeTerminalMemberships = (input) => {
    ...
    const matches = groups
        .map(group => {
        const selector = parseJson(group.selectorDslJson, {});
...
export const getTerminalGroupMemberships = (sandboxId, terminalId) => {
    assertSandboxUsable(sandboxId);
  ...
  `).all(sandboxId, terminalId);
    return {
        ...
        groups: rows.map(item => ({
            ...
            rank: item.rank,
            matchedBy: parseJson(item.matched_by_json, {}),
        })),
    };
};
export const getSelectorGroupMemberships = (input) => {
  ...
  `).all(input.sandboxId, input.groupId);
    return {
...
export const resolveTerminalIdsByScope = (input) => {
    ...
    const terminalIds = terminals
        ...
        .map(item => item.terminalId);
    if (scopeType === 'GROUP') {
        const groupMembershipRows = db.select().from(selectorGroupMembershipsTable).where(and(eq(selectorGroupMembershipsTable.sandboxId, sandboxId))).all();
        return Array.from(new Set(groupMembershipRows
...
export const upsertTerminalRuntimeFacts = (input) => {
    const sandboxId = input.sandboxId;
    assertSandboxUsable(sandboxId);
    const terminal = db.select().from(terminalsTable).where(and(eq(terminalsTable.sandboxId, sandboxId), eq(terminalsTable.terminalId, input.terminalId))).get();
    if (!terminal) {
        throw new Error('终端不存在');
    }
    const previousDeviceInfo = parseJson(terminal.deviceInfoJson, {});
    ...
    const previousRuntimeInfo = typeof previousDeviceInfo.runtimeInfo === 'object' && previousDeviceInfo.runtimeInfo
        ? previousDeviceInfo.runtimeInfo
        : {};
...
export const ensureGroupCanBeDeleted = (sandboxId, groupId) => {
    const policy = db.select().from(projectionPoliciesTable).where(and(eq(projectionPoliciesTable.sandboxId, sandboxId), eq(projectionPoliciesTable.scopeType, 'GROUP'), eq(projectionPoliciesTable.scopeKey, groupId), eq(projectionPoliciesTable.enabled, 1))).get();
    if (policy) {
...
export const deleteSelectorGroup = (input) => {
    ...
    const affectedTerminalIds = db.select().from(selectorGroupMembershipsTable)
        .where(and(eq(selectorGroupMembershipsTable.sandboxId, sandboxId), eq(selectorGroupMembershipsTable.groupId, input.groupId)))
        .all()
        .map(item => item.terminalId);
    const staleGroupProjections = db.select().from(projectionsTable).where(and(eq(projectionsTable.sandboxId, sandboxId), eq(projectionsTable.scopeType, 'GROUP'), eq(projectionsTable.scopeKey, input.groupId))).all();
    ...
    db.delete(projectionPoliciesTable)
        .where(and(eq(projectionPoliciesTable.sandboxId, sandboxId), eq(projectionPoliciesTable.scopeType, 'GROUP'), eq(projectionPoliciesTable.scopeKey, input.groupId)))
        .run();
