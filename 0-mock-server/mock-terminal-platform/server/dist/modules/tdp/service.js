...
const toIsoTime = (timestamp) => new Date(timestamp).toISOString();
const getNextCursorForTerminal = (sandboxId, terminalId) => {
    const row = sqlite.prepare(`
    SELECT COALESCE(MAX(cursor), 0) as high_watermark
    FROM tdp_change_logs
    WHERE sandbox_id = ? AND target_terminal_id = ?
  `).get(sandboxId, terminalId);
...
const pushToOnlineTerminal = (sandboxId, terminalId, message) => {
...
const queueProjectionChangeToOnlineTerminal = (terminalId, input) => {
...
const flushProjectionQueueToOnlineTerminal = (sandboxId, terminalId) => {
    const sessions = listOnlineSessionsByTerminalId(sandboxId, terminalId);
    for (const session of sessions) {
        if (session.socket.readyState !== 1)
            continue;
...
const pushProjectionChangeToOnlineTerminal = (terminalId, input) => {
...
export const listSessions = (sandboxId) => {
...
export const heartbeatSession = (input) => {
...
export const updateSessionAppliedRevision = (input) => {
...
export const acknowledgeSessionRevision = (input) => {
    ...
    if ((input.topic === 'remote.control' || input.topic === 'print.command') && input.itemKey?.trim()) {
        ...
        if (command?.payloadJson) {
            const payload = parseJson(command.payloadJson, {});
...
export const disconnectSession = (input) => {
    const { sandboxId, sessionId } = input;
    assertSandboxUsable(sandboxId);
    const timestamp = now();
    db.update(sessionsTable)
...
export const validateTerminalAccessToken = (input) => {
    const sandboxId = input.sandboxId;
    assertSandboxUsable(sandboxId);
    const timestamp = now();
    ...
    LIMIT 1
  `).get(input.terminalId, input.token);
...
export const listTopics = (sandboxId) => {
    assertSandboxUsable(sandboxId);
...
export const listProjections = (sandboxId) => {
    assertSandboxUsable(sandboxId);
...
export const listChangeLogs = (sandboxId) => {
    assertSandboxUsable(sandboxId);
...
export const listCommandOutbox = (sandboxId) => {
    assertSandboxUsable(sandboxId);
...
export const dispatchTaskReleaseToDataPlane = (input) => {
    const { sandboxId, releaseId } = input;
    assertSandboxUsable(sandboxId);
    const releaseMeta = sqlite.prepare('SELECT task_type, payload_json FROM task_releases WHERE release_id = ? AND sandbox_id = ? LIMIT 1').get(releaseId, sandboxId);
    if (!releaseMeta) {
  ...
  `).all(sandboxId, releaseId, sandboxId);
    const timestamp = now();
    for (const instance of instances) {
        ...
        const existing = sqlite.prepare('SELECT projection_id FROM tdp_projections WHERE sandbox_id = ? AND topic_key = ? AND scope_type = ? AND scope_key = ? AND item_key = ? LIMIT 1').get(sandboxId, 'tcp.task.release', 'TERMINAL', instance.terminal_id, instance.instance_id);
        if (existing) {
...
const dispatchRemoteControlRelease = (sandboxId, releaseId) => {
    const instances = sqlite.prepare(`
    SELECT ti.instance_id, ti.terminal_id, ti.payload_json
    FROM task_instances ti
    JOIN task_releases tr ON tr.release_id = ti.release_id
    WHERE ti.release_id = ? AND tr.sandbox_id = ?
  `).all(releaseId, sandboxId);
    const timestamp = now();
    for (const instance of instances) {
        const commandId = createId('cmd');
        const basePayload = parseJson(instance.payload_json, {});
...
export const getTerminalChanges = (sandboxId, terminalId) => {
    assertSandboxUsable(sandboxId);
    const rows = sqlite.prepare('SELECT change_id, cursor, topic_key, operation, item_key, revision, payload_json, source_release_id, created_at FROM tdp_change_logs WHERE sandbox_id = ? AND target_terminal_id = ? ORDER BY created_at DESC LIMIT 50').all(sandboxId, terminalId);
    return rows.map((item) => ({
...
export const getTerminalChangesSince = (sandboxId, terminalId, cursor, limit = 100) => {
    assertSandboxUsable(sandboxId);
    const rows = sqlite.prepare('SELECT change_id, cursor, topic_key, operation, scope_type, scope_key, item_key, revision, payload_json, source_release_id, created_at FROM tdp_change_logs WHERE sandbox_id = ? AND target_terminal_id = ? AND cursor > ? ORDER BY cursor ASC LIMIT ?').all(sandboxId, terminalId, cursor, limit);
    return rows.map((item) => ({
        cursor: item.cursor,
        change: toProjectionEnvelope({
            topicKey: item.topic_key,
...
export const getHighWatermarkForTerminal = (sandboxId, terminalId) => {
    assertSandboxUsable(sandboxId);
    const row = sqlite.prepare('SELECT COALESCE(MAX(cursor), 0) as high_watermark FROM tdp_change_logs WHERE sandbox_id = ? AND target_terminal_id = ?').get(sandboxId, terminalId);
...
export const listScopes = (sandboxId) => {
    assertSandboxUsable(sandboxId);
    const topicScopes = sqlite.prepare('SELECT key as topic_key, scope_type, COUNT(*) as topic_count FROM tdp_topics WHERE sandbox_id = ? GROUP BY key, scope_type ORDER BY key ASC').all(sandboxId);
    const projectionScopes = sqlite.prepare('SELECT topic_key, scope_type, scope_key, item_key, MAX(revision) as revision, MAX(updated_at) as updated_at FROM tdp_projections WHERE sandbox_id = ? GROUP BY topic_key, scope_type, scope_key, item_key ORDER BY updated_at DESC').all(sandboxId);
...
export const upsertProjectionBatch = (input) => {
    ...
    assertSandboxUsable(sandboxId);
    const timestamp = now();
    const queuedByTerminal = new Map();
    const items = input.projections.map(item => {
        ...
        const existingProjection = sqlite.prepare('SELECT projection_id, revision FROM tdp_projections WHERE sandbox_id = ? AND topic_key = ? AND scope_type = ? AND scope_key = ? AND item_key = ? LIMIT 1').get(sandboxId, item.topicKey, scopeType, item.scopeKey, itemKey);
