...
import { hotUpdatePackagesTable, hotUpdateReleasesTable, terminalsTable, } from '../../database/schema.js';
import { createId, now, parseJson } from '../../shared/utils.js';
import { assertSandboxUsable } from '../sandbox/service.js';
import { createProjectionPolicy, deleteProjectionPolicy, updateProjectionPolicy, } from '../tdp/policyService.js';
...
const ensureStorageDir = (sandboxId) => {
...
const makeDownloadToken = (input) => sha256(`${input.sandboxId}:${input.packageId}:${input.sha256}`);
const buildDownloadUrl = (input) => `/api/v1/hot-updates/packages/${input.packageId}/download?sandboxId=${encodeURIComponent(input.sandboxId)}&token=${encodeURIComponent(makeDownloadToken(input))}`;
const mapPackageRecord = (item) => {
    const manifest = parseJson(item.manifestJson, {});
    return {
        ...item,
        manifest,
...
const mapReleaseRecord = (item) => ({
    ...item,
    enabled: Boolean(item.enabled),
    desiredPayload: parseJson(item.desiredPayloadJson, {}),
});
const getPackageSummary = (sandboxId, packageId) => {
    const record = db.select().from(hotUpdatePackagesTable).where(and(eq(hotUpdatePackagesTable.sandboxId, sandboxId), eq(hotUpdatePackagesTable.packageId, packageId))).get();
    return record ? mapPackageRecord(record) : null;
};
export const listHotUpdatePackages = (sandboxId) => {
...
export const getHotUpdatePackage = (input) => {
    assertSandboxUsable(input.sandboxId);
    const record = db.select().from(hotUpdatePackagesTable).where(and(eq(hotUpdatePackagesTable.sandboxId, input.sandboxId), eq(hotUpdatePackagesTable.packageId, input.packageId))).get();
    if (!record) {
        throw new Error('HOT_UPDATE_PACKAGE_NOT_FOUND');
    }
    return mapPackageRecord(record);
};
export const updateHotUpdatePackageStatus = (input) => {
    const current = getHotUpdatePackage(input);
    const timestamp = now();
    db.update(hotUpdatePackagesTable).set({
        status: requireString(input.status, 'status'),
        updatedAt: timestamp,
    }).where(and(eq(hotUpdatePackagesTable.sandboxId, input.sandboxId), eq(hotUpdatePackagesTable.packageId, input.packageId))).run();
...
export const uploadHotUpdatePackage = (input) => {
    ...
    const duplicated = db.select().from(hotUpdatePackagesTable).where(and(eq(hotUpdatePackagesTable.sandboxId, sandboxId), eq(hotUpdatePackagesTable.appId, manifest.appId), eq(hotUpdatePackagesTable.bundleVersion, manifest.bundleVersion), eq(hotUpdatePackagesTable.runtimeVersion, manifest.runtimeVersion), eq(hotUpdatePackagesTable.sha256, archiveSha256))).get();
    if (duplicated) {
...
export const listHotUpdateReleases = (sandboxId) => {
    assertSandboxUsable(sandboxId);
...
export const getHotUpdateRelease = (input) => {
    assertSandboxUsable(input.sandboxId);
    const record = db.select().from(hotUpdateReleasesTable).where(and(eq(hotUpdateReleasesTable.sandboxId, input.sandboxId), eq(hotUpdateReleasesTable.releaseId, input.releaseId))).get();
    if (!record) {
...
const estimateMatchedTerminalCount = (input) => {
    if (input.scopeType === 'TERMINAL') {
        const exists = db.select().from(terminalsTable).where(and(eq(terminalsTable.sandboxId, input.sandboxId), eq(terminalsTable.terminalId, input.scopeKey))).get();
        return exists ? 1 : 0;
    }
...
export const activateHotUpdateRelease = (input) => {
    ...
    sqlite.transaction(() => {
        ...
        db.update(hotUpdateReleasesTable).set({
            ...
            desiredPayloadJson: JSON.stringify(desiredPayload),
            updatedAt: now(),
        }).where(and(eq(hotUpdateReleasesTable.sandboxId, input.sandboxId), eq(hotUpdateReleasesTable.releaseId, input.releaseId))).run();
    })();
    return {
...
export const pauseHotUpdateRelease = (input) => updateReleaseLifecycle({
    ...input,
    status: 'PAUSED',
    rolloutMode: 'paused',
    enabled: false,
});
export const cancelHotUpdateRelease = (input) => {
    const current = getHotUpdateRelease(input);
    sqlite.transaction(() => {
        if (current.policyId) {
