...
import { createSandbox, getRuntimeContext, listAuditLogs, listSandboxes, getPlatformOverview, prepareKernelBaseTestSandbox, switchCurrentSandbox, updateSandbox, } from '../sandbox/service.js';
import { activateTerminal, batchCreateTerminals, createActivationCodes, createTaskInstancesForRelease, createTaskRelease, deactivateTerminal, forceTerminalStatus, getTaskTrace, listActivationCodes, listProfiles, listTaskInstances, listTaskReleases, listTemplates, listTerminals, refreshTerminalToken, reportTaskResult, updateDeliveryStatus, } from '../tcp/service.js';
import { connectSession, createTopic, disconnectSession, dispatchTaskReleaseToDataPlane, forceCloseSession, getHighWatermarkForTerminal, getTerminalChangesSince, getTerminalSnapshot, heartbeatSession, listChangeLogs, listCommandOutbox, listProjections, listScopes, listSessions, listTopics, sendEdgeDegradedToSession, sendProtocolErrorToSession, sendSessionRehomeRequired, upsertProjection, upsertProjectionBatch, } from '../tdp/service.js';
import { createSelectorGroup, deleteSelectorGroup, getSelectorGroupMemberships, getTerminalGroupMemberships, listSelectorGroups, recomputeTerminalMemberships, resolveTerminalIdsByScope, updateSelectorGroup, upsertTerminalRuntimeFacts, } from '../tdp/groupService.js';
import { createProjectionPolicy, getProjectionPolicy, deleteProjectionPolicy, listProjectionPolicies, updateProjectionPolicy, } from '../tdp/policyService.js';
import { getPolicyCenterOverview, getSelectorGroupStats, listSelectorGroupPolicies, previewSelectorGroup, validateProjectionPolicyDraft, } from '../tdp/policyCenterService.js';
import { getTerminalDecisionTrace, getTerminalResolvedTopics, getTerminalTopicDecision, previewPolicyImpact, } from '../tdp/decisionService.js';
...
import { createBrand, createContract, createPlatform, createProfile, createProject, createStore, createTemplate, createTenant, deleteBrand, deleteContract, deletePlatform, deleteProfile, deleteProject, deleteStore, deleteTemplate, deleteTenant, listBrands, listContracts, listPlatforms, listProfiles as listMasterProfiles, listProjects, listStores, listTemplates as listMasterTemplates, listTenants, updateBrand, updateContract, updatePlatform, updateProfile, updateProject, updateStore, updateTemplate, updateTenant, } from '../master-data/service.js';
import { activateHotUpdateRelease, cancelHotUpdateRelease, createHotUpdateRelease, getHotUpdatePackage, getHotUpdateRelease, listHotUpdatePackages, listHotUpdateReleases, pauseHotUpdateRelease, previewHotUpdateReleaseImpact, resolveHotUpdateDownload, updateHotUpdatePackageStatus, uploadHotUpdatePackage, } from './hotUpdateService.js';
import { listHotUpdateVersionDrift, listTerminalVersionHistory, reportTerminalVersion, } from './hotUpdateVersionReportService.js';
export const createRouter = () => {
    const router = Router();
    const withBadRequest = (handler) => (req, res, next) => {
        try {
    ...
    const requireBodySandboxId = (body) => {
    ...
    const requireQuerySandboxId = (query) => {
    ...
    router.get('/api/v1/admin/export', withBadRequest((req, res) => ok(res, exportMockData(requireQuerySandboxId(req.query)))));
    ...
    router.get('/api/v1/admin/export/download', (req, res) => {
        const content = exportMockDataText(requireQuerySandboxId(req.query));
    ...
    router.get('/api/v1/admin/hot-updates/packages', (req, res) => {
        try {
            return ok(res, listHotUpdatePackages(requireQuerySandboxId(req.query)));
        }
        catch (error) {
    ...
    router.get('/api/v1/admin/hot-updates/packages/:packageId', (req, res) => {
        try {
            return ok(res, getHotUpdatePackage({ sandboxId: requireQuerySandboxId(req.query), packageId: req.params.packageId }));
        }
        catch (error) {
    ...
    router.get('/api/v1/hot-updates/packages/:packageId/download', (req, res) => {
        try {
            const sandboxId = requireQuerySandboxId(req.query);
    ...
    router.get('/api/v1/admin/hot-updates/releases', (req, res) => {
        try {
            return ok(res, listHotUpdateReleases(requireQuerySandboxId(req.query)));
        }
        catch (error) {
    ...
    router.get('/api/v1/admin/hot-updates/releases/:releaseId', (req, res) => {
        try {
            return ok(res, getHotUpdateRelease({ sandboxId: requireQuerySandboxId(req.query), releaseId: req.params.releaseId }));
        }
        catch (error) {
    ...
    router.get('/api/v1/admin/terminals/:terminalId/version-history', (req, res) => {
        try {
            return ok(res, listTerminalVersionHistory({
                sandboxId: requireQuerySandboxId(req.query),
    ...
    router.get('/api/v1/admin/hot-updates/version-drift', (req, res) => {
        try {
            return ok(res, listHotUpdateVersionDrift(requireQuerySandboxId(req.query)));
        }
        catch (error) {
            return fail(res, error instanceof Error ? error.message : '查询版本漂移失败', 400);
        }
    });
    router.get('/api/v1/admin/terminals', (req, res) => ok(res, listTerminals(requireQuerySandboxId(req.query))));
    router.get('/api/v1/admin/profiles', (req, res) => ok(res, listProfiles(requireQuerySandboxId(req.query))));
    router.get('/api/v1/admin/templates', (req, res) => ok(res, listTemplates(requireQuerySandboxId(req.query))));
    router.get('/api/v1/admin/activation-codes', (req, res) => ok(res, listActivationCodes(requireQuerySandboxId(req.query))));
    router.get('/api/v1/admin/master-data/platforms', withBadRequest((req, res) => ok(res, listPlatforms(requireQuerySandboxId(req.query)))));
    ...
    router.delete('/api/v1/admin/master-data/platforms/:platformId', (req, res) => {
        try {
            const result = deletePlatform(requireQuerySandboxId(req.query), req.params.platformId);
    ...
    router.get('/api/v1/admin/master-data/tenants', withBadRequest((req, res) => ok(res, listTenants(requireQuerySandboxId(req.query)))));
    ...
    router.delete('/api/v1/admin/master-data/tenants/:tenantId', (req, res) => {
        try {
            const result = deleteTenant(requireQuerySandboxId(req.query), req.params.tenantId);
    ...
    router.get('/api/v1/admin/master-data/brands', withBadRequest((req, res) => ok(res, listBrands(requireQuerySandboxId(req.query)))));
    ...
    router.delete('/api/v1/admin/master-data/brands/:brandId', (req, res) => {
        try {
            const result = deleteBrand(requireQuerySandboxId(req.query), req.params.brandId);
    ...
    router.get('/api/v1/admin/master-data/projects', withBadRequest((req, res) => ok(res, listProjects(requireQuerySandboxId(req.query)))));
    ...
    router.delete('/api/v1/admin/master-data/projects/:projectId', (req, res) => {
        try {
            const result = deleteProject(requireQuerySandboxId(req.query), req.params.projectId);
    ...
    router.get('/api/v1/admin/master-data/stores', withBadRequest((req, res) => ok(res, listStores(requireQuerySandboxId(req.query)))));
    ...
    router.delete('/api/v1/admin/master-data/stores/:storeId', (req, res) => {
        try {
            const result = deleteStore(requireQuerySandboxId(req.query), req.params.storeId);
    ...
    router.get('/api/v1/admin/master-data/contracts', withBadRequest((req, res) => ok(res, listContracts(requireQuerySandboxId(req.query)))));
    ...
    router.delete('/api/v1/admin/master-data/contracts/:contractId', (req, res) => {
        try {
            const result = deleteContract(requireQuerySandboxId(req.query), req.params.contractId);
    ...
    router.get('/api/v1/admin/master-data/profiles', withBadRequest((req, res) => ok(res, listMasterProfiles(requireQuerySandboxId(req.query)))));
    ...
    router.delete('/api/v1/admin/master-data/profiles/:profileId', (req, res) => {
        try {
            const result = deleteProfile(requireQuerySandboxId(req.query), req.params.profileId);
    ...
    router.get('/api/v1/admin/master-data/templates', withBadRequest((req, res) => ok(res, listMasterTemplates(requireQuerySandboxId(req.query)))));
    ...
    router.delete('/api/v1/admin/master-data/templates/:templateId', (req, res) => {
        try {
            const result = deleteTemplate(requireQuerySandboxId(req.query), req.params.templateId);
    ...
    router.get('/api/v1/admin/tasks/releases', withBadRequest((req, res) => ok(res, listTaskReleases(requireQuerySandboxId(req.query)))));
    router.get('/api/v1/admin/tasks/instances', withBadRequest((req, res) => ok(res, listTaskInstances(requireQuerySandboxId(req.query)))));
    router.get('/api/v1/admin/tasks/instances/:instanceId/trace', (req, res) => {
        try {
            return ok(res, getTaskTrace(requireQuerySandboxId(req.query), req.params.instanceId));
        }
        catch (error) {
    ...
    router.get('/api/v1/admin/tdp/sessions', withBadRequest((req, res) => ok(res, listSessions(requireQuerySandboxId(req.query)))));
    ...
    router.get('/api/v1/admin/tdp/topics', withBadRequest((req, res) => ok(res, listTopics(requireQuerySandboxId(req.query)))));
    ...
    router.get('/api/v1/admin/tdp/scopes', withBadRequest((req, res) => ok(res, listScopes(requireQuerySandboxId(req.query)))));
    router.get('/api/v1/admin/tdp/policy-center/overview', withBadRequest((req, res) => ok(res, getPolicyCenterOverview(requireQuerySandboxId(req.query)))));
    router.get('/api/v1/admin/tdp/groups', withBadRequest((req, res) => ok(res, listSelectorGroups(requireQuerySandboxId(req.query)))));
    ...
    router.post('/api/v1/admin/tdp/groups/recompute-by-scope', (req, res) => {
        try {
            ...
            const scopeKeys = Array.isArray(req.body?.scopeKeys) ? req.body.scopeKeys.filter((item) => typeof item === 'string' && item.trim().length > 0) : [];
    ...
    router.get('/api/v1/admin/tdp/policies', withBadRequest((req, res) => ok(res, listProjectionPolicies(requireQuerySandboxId(req.query)))));
    ...
    router.get('/api/v1/admin/tdp/projections', withBadRequest((req, res) => ok(res, listProjections(requireQuerySandboxId(req.query)))));
    router.get('/api/v1/admin/tdp/change-logs', withBadRequest((req, res) => ok(res, listChangeLogs(requireQuerySandboxId(req.query)))));
    router.get('/api/v1/admin/tdp/commands', withBadRequest((req, res) => ok(res, listCommandOutbox(requireQuerySandboxId(req.query)))));
    ...
    router.get('/mock-admin/fault-rules', withBadRequest((req, res) => ok(res, listFaultRules(requireQuerySandboxId(req.query)))));
