...
import { activationCodesTable, credentialsTable, taskInstancesTable, taskReleasesTable, storesTable, terminalProfilesTable, terminalTemplatesTable, terminalsTable, } from '../../database/schema.js';
import { createId, now, parseJson } from '../../shared/utils.js';
import { assertSandboxUsable } from '../sandbox/service.js';
export const listTerminals = (sandboxId) => {
...
export const listActivationCodes = (sandboxId) => {
    assertSandboxUsable(sandboxId);
...
export const refreshTerminalToken = (input) => {
    const sandboxId = input.sandboxId;
    assertSandboxUsable(sandboxId);
...
export const listTaskReleases = (sandboxId) => {
    assertSandboxUsable(sandboxId);
...
export const listTaskInstances = (sandboxId) => {
    assertSandboxUsable(sandboxId);
...
export const createTaskInstancesForRelease = (input) => {
    ...
    const targetSelector = parseJson(release.targetSelectorJson, {});
...
export const updateDeliveryStatus = (instanceId, deliveryStatus, error) => {
    const timestamp = now();
...
export const reportTaskResult = (sandboxId, instanceId, input) => {
    assertSandboxUsable(sandboxId);
...
export const getTaskTrace = (sandboxId, instanceId) => {
    assertSandboxUsable(sandboxId);
    const instance = sqlite.prepare('SELECT * FROM task_instances WHERE instance_id = ?').get(instanceId);
    if (!instance) {
        throw new Error('任务实例不存在');
    }
    const release = sqlite.prepare('SELECT * FROM task_releases WHERE release_id = ?').get(instance.release_id);
    if (release?.sandbox_id !== sandboxId) {
        throw new Error('任务实例不属于当前沙箱');
    }
    const projections = sqlite.prepare('SELECT * FROM tdp_projections WHERE topic_key = ? AND item_key = ? ORDER BY updated_at DESC').all('tcp.task.release', instance.instance_id);
    const changes = sqlite.prepare('SELECT * FROM tdp_change_logs WHERE topic_key = ? AND item_key = ? ORDER BY created_at DESC LIMIT 20').all('tcp.task.release', instance.instance_id);
    return {
        instance: {
            ...instance,
...
export const batchCreateTerminals = (sandboxId, count) => {
    assertSandboxUsable(sandboxId);
    const timestamp = now();
    const terminalIds = [];
...
export const forceTerminalStatus = (sandboxId, terminalId, input) => {
    assertSandboxUsable(sandboxId);
