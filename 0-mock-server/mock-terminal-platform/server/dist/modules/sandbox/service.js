...
import { auditLogsTable, brandsTable, faultRulesTable, contractsTable, platformsTable, projectsTable, runtimeContextTable, sandboxesTable, tenantsTable, terminalProfilesTable, terminalTemplatesTable, storesTable, topicsTable, } from '../../database/schema.js';
...
const mapSandbox = (item, currentSandboxId) => ({
...
export const getCurrentSandboxId = () => getRuntimeContext().currentSandboxId;
export const getSandboxById = (sandboxId) => db.select().from(sandboxesTable).where(eq(sandboxesTable.sandboxId, sandboxId)).get();
export const assertSandboxExists = (sandboxId) => {
    if (!sandboxId?.trim()) {
...
export const assertSandboxUsable = (sandboxId) => {
...
export const listSandboxes = () => {
    ...
    return db
        ...
        .all()
        .map((item) => mapSandbox(item, currentSandboxId));
};
export const switchCurrentSandbox = (sandboxId) => {
    const sandbox = getSandboxById(sandboxId);
    if (!sandbox) {
        throw new Error('沙箱不存在');
    }
...
export const createSandbox = (input) => {
    ...
    const exists = sqlite.prepare('SELECT sandbox_id FROM sandboxes WHERE LOWER(name) = LOWER(?) LIMIT 1').get(name);
    if (exists) {
    ...
    return mapSandbox(getSandboxById(sandboxId), getCurrentSandboxId());
};
export const updateSandbox = (sandboxId, input) => {
    const sandbox = getSandboxById(sandboxId);
    if (!sandbox) {
        throw new Error('沙箱不存在');
    }
...
export const getPlatformOverview = () => {
  ...
  `).get(sandboxId);
    ...
    WHERE sandbox_id = ?
  `).get(sandboxId);
    const topicStats = sqlite.prepare('SELECT COUNT(*) as total FROM tdp_topics WHERE sandbox_id = ?').get(sandboxId);
    const faultStats = sqlite.prepare('SELECT COUNT(*) as total, SUM(hit_count) as hits FROM fault_rules WHERE sandbox_id = ?').get(sandboxId);
    return {
        sandboxId,
        terminalStats,
        taskStats,
...
export const listAuditLogs = (pagination) => {
    const sandboxId = getCurrentSandboxId();
    const rows = sqlite.prepare('SELECT * FROM audit_logs WHERE sandbox_id = ? ORDER BY created_at DESC LIMIT 500').all(sandboxId);
...
const cloneBaselineData = (sourceSandboxId, targetSandboxId, timestamp) => {
...
const insertKernelBaseTestSandbox = (timestamp) => {
    db.insert(sandboxesTable).values({
...
const deleteSandboxRows = (sandboxId) => {
...
const seedKernelBaseTestSandboxData = (timestamp) => {
    sqlite.prepare(`
    INSERT INTO platforms (platform_id, sandbox_id, platform_code, platform_name, status, description, created_at, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `).run('platform-kernel-base-test', KERNEL_BASE_TEST_SANDBOX_ID, 'PLATFORM_KERNEL_BASE_TEST', 'Kernel Base Test Platform', 'ACTIVE', 'kernel-base 联调测试平台', timestamp, timestamp);
    sqlite.prepare(`
    INSERT INTO tenants (tenant_id, sandbox_id, platform_id, tenant_code, tenant_name, status, description, created_at, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run('tenant-kernel-base-test', KERNEL_BASE_TEST_SANDBOX_ID, 'platform-kernel-base-test', 'TENANT_KERNEL_BASE_TEST', 'Kernel Base Test Tenant', 'ACTIVE', 'kernel-base 联调测试租户', timestamp, timestamp);
    sqlite.prepare(`
    INSERT INTO brands (brand_id, sandbox_id, platform_id, brand_code, brand_name, status, description, created_at, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run('brand-kernel-base-test', KERNEL_BASE_TEST_SANDBOX_ID, 'platform-kernel-base-test', 'BRAND_KERNEL_BASE_TEST', 'Kernel Base Test Brand', 'ACTIVE', 'kernel-base 联调测试品牌', timestamp, timestamp);
    sqlite.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run('project-kernel-base-test', KERNEL_BASE_TEST_SANDBOX_ID, 'platform-kernel-base-test', 'PROJECT_KERNEL_BASE_TEST', 'Kernel Base Test Project', 'ACTIVE', 'kernel-base 联调测试项目', 'SZ', 'Asia/Shanghai', timestamp, timestamp);
    sqlite.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run('store-kernel-base-test', KERNEL_BASE_TEST_SANDBOX_ID, 'platform-kernel-base-test', 'tenant-kernel-base-test', 'brand-kernel-base-test', 'project-kernel-base-test', 'KB001', 'STORE_KERNEL_BASE_TEST', 'Kernel Base Test Store', 'ACTIVE', 'kernel-base 联调测试门店', '深圳市南山区科苑南路', 'Kernel Tester', '13800138000', timestamp, timestamp);
    sqlite.prepare(`
  ...
  `).run('contract-kernel-base-test', KERNEL_BASE_TEST_SANDBOX_ID, 'platform-kernel-base-test', 'project-kernel-base-test', 'tenant-kernel-base-test', 'brand-kernel-base-test', 'store-kernel-base-test', 'CONTRACT_KERNEL_BASE_TEST', 'KB001', '2026-01-01', '2026-12-31', 'ACTIVE', 'kernel-base 联调测试合同', timestamp, timestamp);
    sqlite.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `).run('profile-kernel-base-android-pos', KERNEL_BASE_TEST_SANDBOX_ID, 'KERNEL_BASE_ANDROID_POS', 'Kernel Base Android POS', 'kernel-base 联调测试终端机型', serializeJson({
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run('template-kernel-base-android-pos-standard', KERNEL_BASE_TEST_SANDBOX_ID, 'KERNEL_BASE_ANDROID_POS_STANDARD', 'Kernel Base Android POS Standard', 'kernel-base 联调测试终端模板', 'profile-kernel-base-android-pos', serializeJson({
        app: { theme: 'light', locale: 'zh-CN' },
        hardware: { printer: true, scanner: true },
    }), serializeJson(['kernel-base-test', 'android-pos']), timestamp, timestamp);
    for (let index = 1; index <= 12; index += 1) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(`2000000000${String(index).padStart(2, '0')}`, KERNEL_BASE_TEST_SANDBOX_ID, 'platform-kernel-base-test', 'tenant-kernel-base-test', 'brand-kernel-base-test', 'project-kernel-base-test', 'store-kernel-base-test', 'profile-kernel-base-android-pos', 'template-kernel-base-android-pos-standard', 'AVAILABLE', timestamp + 30 * 24 * 3600_000, timestamp);
    }
    const topics = [
        {
