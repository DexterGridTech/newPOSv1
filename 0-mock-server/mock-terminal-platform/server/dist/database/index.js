...
const resolveDataFile = (override) => override?.trim()
...
export const resetDatabaseConnection = (input) => {
    sqlite.close();
...
const migrateTdpProjectionIdentity = () => {
  ...
  `).all();
    ...
    const transaction = sqlite.transaction((items) => {
        items.forEach(item => {
            update.run(buildLegacyProjectionItemKey({
...
const migratePlatformScopedMasterData = () => {
    const timestamp = now();
    const sandboxRows = sqlite.prepare('SELECT sandbox_id FROM sandboxes').all();
    for (const row of sandboxRows) {
        const existing = sqlite.prepare('SELECT platform_id FROM platforms WHERE sandbox_id = ? LIMIT 1').get(row.sandbox_id);
        if (!existing) {
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
      `).run(platformId, row.sandbox_id, row.sandbox_id === DEFAULT_SANDBOX_ID ? 'PLATFORM_DEFAULT' : `PLATFORM_${platformId.replace(/-/g, '_').toUpperCase()}`, row.sandbox_id === DEFAULT_SANDBOX_ID ? '默认集团平台' : `平台 ${platformId}`, 'ACTIVE', '由系统迁移生成的平台主数据', timestamp, timestamp);
        }
    }
    sqlite.exec(`
    UPDATE tenants
  ...
  `);
    const contractsCount = sqlite.prepare('SELECT COUNT(*) as count FROM contracts').get();
    if (contractsCount.count === 0) {
        const stores = sqlite.prepare('SELECT * FROM stores ORDER BY created_at ASC').all();
      ...
      `).run(contractId, store.sandbox_id, store.platform_id, store.project_id, store.tenant_id, store.brand_id, store.store_id, `CONTRACT_${store.store_code}`, store.unit_code || store.store_code, '2026-01-01', '2026-12-31', 'ACTIVE', `门店 ${store.store_name} 的默认迁移合同`, store.created_at ?? timestamp, store.updated_at ?? timestamp);
        }
    }
};
const migrateMasterDataToIndependentModel = () => {
    const brandColumns = sqlite.prepare('PRAGMA table_info(brands)').all();
...
const ensureRuntimeContext = () => {
    const timestamp = now();
    const current = sqlite.prepare('SELECT current_sandbox_id FROM platform_runtime_context WHERE context_key = ?').get(RUNTIME_CONTEXT_KEY);
    if (!current) {
    ...
    const sandbox = sqlite.prepare('SELECT sandbox_id, status FROM sandboxes WHERE sandbox_id = ?').get(current.current_sandbox_id);
...
const seedDefaultData = () => {
    const count = sqlite.prepare('SELECT COUNT(*) as count FROM sandboxes').get();
    if (count.count > 0)
        return;
    const timestamp = now();
    sqlite.prepare(`
  ...
  `).run(DEFAULT_SANDBOX_ID, '默认联调沙箱', '面向 TDP/TCP 联调的默认沙箱', 'ACTIVE', 1, 'EMPTY', null, 20260406, DEFAULT_OWNER_USER_ID, DEFAULT_OWNER_TEAM_ID, 'integration', JSON.stringify({ maxTerminals: 200, maxTasks: 1000, maxFaultRules: 100, maxStorageSize: '2GB' }), 'system', timestamp, timestamp);
    sqlite.prepare(`
    INSERT INTO platforms (platform_id, sandbox_id, platform_code, platform_name, status, description, created_at, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `).run('platform-default', DEFAULT_SANDBOX_ID, 'PLATFORM_DEFAULT', '默认购物中心集团', 'ACTIVE', '默认联调平台', timestamp, timestamp);
    sqlite.prepare(`
    INSERT INTO tenants (tenant_id, sandbox_id, platform_id, tenant_code, tenant_name, status, description, created_at, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run('tenant-mixc', DEFAULT_SANDBOX_ID, 'platform-default', 'TENANT_MIXC', '万象城租户', 'ACTIVE', '默认联调租户', timestamp, timestamp);
    sqlite.prepare(`
    INSERT INTO brands (brand_id, sandbox_id, platform_id, brand_code, brand_name, status, description, created_at, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run('brand-mixc', DEFAULT_SANDBOX_ID, 'platform-default', 'BRAND_MIXC', '万象城品牌', 'ACTIVE', '默认联调品牌', timestamp, timestamp);
    sqlite.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run('project-mixc-bay', DEFAULT_SANDBOX_ID, 'platform-default', 'PROJECT_MIXC_BAY', '深圳湾万象城项目', 'ACTIVE', '默认联调项目', 'SZ', 'Asia/Shanghai', timestamp, timestamp);
    sqlite.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run('store-sz-bay', DEFAULT_SANDBOX_ID, 'platform-default', 'tenant-mixc', 'brand-mixc', 'project-mixc-bay', 'L0102', 'STORE_SZ_BAY', '深圳湾旗舰店', 'ACTIVE', '默认联调门店', '深圳市南山区', '张三', '13800000000', timestamp, timestamp);
    sqlite.prepare(`
    INSERT INTO stores (store_id, sandbox_id, platform_id, tenant_id, brand_id, project_id, unit_code, store_code, store_name, status, description, address, contact_name, contact_phone, created_at, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run('store-gz-tower', DEFAULT_SANDBOX_ID, 'platform-default', 'tenant-mixc', 'brand-mixc', 'project-mixc-bay', 'L0203', 'STORE_GZ_TOWER', '广州塔体验店', 'ACTIVE', '第二个默认联调门店', '广州市海珠区', '李四', '13900000000', timestamp, timestamp);
    sqlite.prepare(`
    INSERT INTO contracts (
      contract_id, sandbox_id, platform_id, project_id, tenant_id, brand_id, store_id,
      contract_code, unit_code, start_date, end_date, status, description, created_at, updated_at
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run('contract-sz-bay-main', DEFAULT_SANDBOX_ID, 'platform-default', 'project-mixc-bay', 'tenant-mixc', 'brand-mixc', 'store-sz-bay', 'CONTRACT_SZ_BAY_MAIN', 'L0102', '2026-01-01', '2026-12-31', 'ACTIVE', '深圳湾旗舰店默认合同', timestamp, timestamp);
    sqlite.prepare(`
    INSERT INTO contracts (
      contract_id, sandbox_id, platform_id, project_id, tenant_id, brand_id, store_id,
      contract_code, unit_code, start_date, end_date, status, description, created_at, updated_at
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run('contract-gz-tower-main', DEFAULT_SANDBOX_ID, 'platform-default', 'project-mixc-bay', 'tenant-mixc', 'brand-mixc', 'store-gz-tower', 'CONTRACT_GZ_TOWER_MAIN', 'L0203', '2026-01-01', '2026-12-31', 'ACTIVE', '广州塔体验店默认合同', timestamp, timestamp);
    sqlite.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `).run('profile-android-pos', DEFAULT_SANDBOX_ID, 'ANDROID_POS', '安卓收银机', '适用于前台收银，支持扫码、打印、小票外设', JSON.stringify({ supportsDualScreen: true, supportsCamera: true, supportsPrinter: true, supportsScanner: true }), timestamp, timestamp);
    sqlite.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run('terminal-template-android-pos-standard', DEFAULT_SANDBOX_ID, 'ANDROID_POS_STANDARD', '安卓收银机标准模板', '收银场景标准配置，适合商场零售门店', 'profile-android-pos', JSON.stringify({ app: { theme: 'dark', locale: 'zh-CN' }, hardware: { printer: true, scanner: true } }), JSON.stringify(['android-pos', 'cashier', 'standard']), timestamp, timestamp);
    ...
    `).run(terminalId, DEFAULT_SANDBOX_ID, 'platform-default', 'project-mixc-bay', 'tenant-mixc', 'brand-mixc', `store-${index <= 3 ? 'sz-bay' : 'gz-tower'}`, 'profile-android-pos', 'terminal-template-android-pos-standard', 'ACTIVE', index <= 4 ? 'ONLINE' : 'OFFLINE', index === 5 ? 'WARNING' : 'HEALTHY', '2.3.18', 'bundle-2026.04.06', 'config-2026.04.01', `fp-${terminalId}`, JSON.stringify({ model: 'PDA-X1', osVersion: 'Android 14', manufacturer: 'IMPOS2' }), 'STANDARD', timestamp - index * 3600_000, timestamp - index * 60_000, timestamp, timestamp);
    }
    ...
    INSERT INTO activation_codes (code, sandbox_id, platform_id, tenant_id, brand_id, project_id, store_id, profile_id, template_id, status, expires_at, created_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(`10000000000${index}`, DEFAULT_SANDBOX_ID, 'platform-default', 'tenant-mixc', 'brand-mixc', 'project-mixc-bay', index <= 4 ? 'store-sz-bay' : 'store-gz-tower', 'profile-android-pos', 'terminal-template-android-pos-standard', index <= 2 ? 'USED' : 'AVAILABLE', timestamp + 7 * 24 * 3600_000, timestamp);
    }
    sqlite.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(createId('topic'), DEFAULT_SANDBOX_ID, 'tcp.task.release', 'TCP 任务发布主题', 'FLEXIBLE_JSON', JSON.stringify({ type: 'object', required: ['taskType', 'releaseId', 'payload'] }), 'TERMINAL', 72, timestamp, timestamp);
    sqlite.prepare(`
    INSERT INTO tdp_topics (topic_id, sandbox_id, key, name, payload_mode, schema_json, scope_type, retention_hours, created_at, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(createId('topic'), DEFAULT_SANDBOX_ID, 'remote.control', '远程控制命令主题', 'EPHEMERAL_COMMAND', JSON.stringify({ type: 'object', required: ['commandType'] }), 'TERMINAL', 1, timestamp, timestamp);
    sqlite.prepare(`
    INSERT INTO tdp_topics (topic_id, sandbox_id, key, name, payload_mode, schema_json, scope_type, retention_hours, created_at, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(createId('topic'), DEFAULT_SANDBOX_ID, 'terminal.config.state', '终端配置状态主题', 'FLEXIBLE_JSON', JSON.stringify({ type: 'object', required: ['configVersion'] }), 'TERMINAL', 168, timestamp, timestamp);
    sqlite.prepare(`
    INSERT INTO tdp_topics (topic_id, sandbox_id, key, name, payload_mode, schema_json, scope_type, retention_hours, created_at, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(createId('topic'), DEFAULT_SANDBOX_ID, 'terminal.group.membership', 'Terminal Group Membership', 'FLEXIBLE_JSON', JSON.stringify({ type: 'object', required: ['membershipVersion', 'groups'] }), 'TERMINAL', 168, timestamp, timestamp);
    sqlite.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(createId('projection'), DEFAULT_SANDBOX_ID, 'terminal.config.state', 'TERMINAL', 'T-1001', 'terminal.config.state:T-1001', 4, JSON.stringify({ configVersion: 'config-2026.04.01', featureFlags: { mockMode: true, grayUpgrade: true } }), timestamp);
    sqlite.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(createId('change'), DEFAULT_SANDBOX_ID, 1, 'terminal.config.state', 'upsert', 'TERMINAL', 'T-1001', 'terminal.config.state:T-1001', 'T-1001', 4, JSON.stringify({ configVersion: 'config-2026.04.01', source: 'release_bootstrap' }), 'release_bootstrap', timestamp);
    sqlite.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run('release_bootstrap', DEFAULT_SANDBOX_ID, 'CONFIG_PUBLISH', 'CONFIG', 'config-2026.04.01', '默认配置下发', JSON.stringify({ type: 'STORE', ids: ['store-sz-bay'] }), JSON.stringify({ configVersion: 'config-2026.04.01' }), 60, 'COMPLETED', 'APPROVED', timestamp, timestamp);
    sqlite.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run('instance_bootstrap', 'release_bootstrap', 'T-1001', 'CONFIG_PUBLISH', 'COMPLETED', 'ACKED', JSON.stringify({ configVersion: 'config-2026.04.01' }), JSON.stringify({ success: true }), timestamp, timestamp, timestamp, timestamp);
    sqlite.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `).run('session_bootstrap', 'T-1001', DEFAULT_SANDBOX_ID, '2.3.18', 'tdp/1.0', 'CONNECTED', timestamp - 3_600_000, timestamp - 60_000);
    sqlite.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run('fault_bootstrap_delay', DEFAULT_SANDBOX_ID, '配置下发延迟 3 秒', 'TDP_DELIVERY', JSON.stringify({ taskType: 'CONFIG_PUBLISH' }), JSON.stringify({ type: 'DELAY', durationMs: 3000 }), 1, 2, timestamp, timestamp);
    sqlite.prepare(`
    INSERT INTO audit_logs (audit_id, sandbox_id, domain, action, operator, target_id, detail_json, created_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `).run(createId('audit'), DEFAULT_SANDBOX_ID, 'sandbox', 'bootstrap', 'system', DEFAULT_SANDBOX_ID, JSON.stringify({ message: '默认联调沙箱已初始化' }), timestamp);
};
