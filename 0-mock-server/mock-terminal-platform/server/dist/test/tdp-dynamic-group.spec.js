...
describe('mock-terminal-platform TDP dynamic group', () => {
    ...
    it('recomputes memberships when runtime facts change through TDP handshake', async () => {
        ...
        const membershipsBeforeResponse = await fetch(`${server.getHttpBaseUrl()}/api/v1/admin/tdp/terminals/${terminalId}/memberships?sandboxId=${sandboxId}`);
        const membershipsBeforePayload = await membershipsBeforeResponse.json();
        ...
        const membershipsAfterResponse = await fetch(`${server.getHttpBaseUrl()}/api/v1/admin/tdp/terminals/${terminalId}/memberships?sandboxId=${sandboxId}`);
        const membershipsAfterPayload = await membershipsAfterResponse.json();
    ...
    it('materializes group policy into projection and terminal snapshot', async () => {
        ...
        const projectionsPayload = await projectionsResponse.json();
        expect(projectionsPayload.data.some(item => item.topicKey === 'kernel.workflow.definition'
            && item.scopeType === 'GROUP'
            && item.scopeKey === groupPayload.data.groupId
            && item.itemKey === 'workflow.remote.gray')).toBe(true);
        ...
        expect(snapshotPayload.data.some(item => item.topic === 'kernel.workflow.definition'
            && item.scopeType === 'GROUP'
    ...
    it('keeps existing terminal snapshots valid when a later terminal newly joins the same group policy', async () => {
        ...
        const preparePayload = await prepareResponse.json();
        const sandboxId = preparePayload.data.sandboxId;
        const activate = async (activationCode, deviceId) => {
        ...
        const snapshotFirstBefore = await fetch(`${server.getHttpBaseUrl()}/api/v1/tdp/terminals/${terminalOne.data.terminalId}/snapshot?sandboxId=${sandboxId}`);
        const snapshotFirstBeforePayload = await snapshotFirstBefore.json();
        expect(snapshotFirstBeforePayload.data.some(item => item.topic === 'config.delta'
            && item.scopeType === 'GROUP'
            && item.itemKey === 'config.dynamic.join')).toBe(true);
        ...
        const snapshotFirstAfter = await fetch(`${server.getHttpBaseUrl()}/api/v1/tdp/terminals/${terminalOne.data.terminalId}/snapshot?sandboxId=${sandboxId}`);
        const snapshotFirstAfterPayload = await snapshotFirstAfter.json();
        const snapshotSecondAfter = await fetch(`${server.getHttpBaseUrl()}/api/v1/tdp/terminals/${terminalTwo.data.terminalId}/snapshot?sandboxId=${sandboxId}`);
        const snapshotSecondAfterPayload = await snapshotSecondAfter.json();
        expect(snapshotFirstAfterPayload.data.some(item => item.topic === 'config.delta'
            && item.scopeType === 'GROUP'
            && item.itemKey === 'config.dynamic.join')).toBe(true);
        expect(snapshotSecondAfterPayload.data.some(item => item.topic === 'config.delta'
            && item.scopeType === 'GROUP'
            && item.itemKey === 'config.dynamic.join')).toBe(true);
    });
    it('recompute-all recalculates memberships for every terminal in a sandbox', async () => {
        ...
        const recomputeAllPayload = await recomputeAllResponse.json();
    ...
    it('rejects deleting a group while an enabled policy is still bound', async () => {
        ...
        expect(policyResponse.status).toBe(201);
        const deleteGroupResponse = await fetch(`${server.getHttpBaseUrl()}/api/v1/admin/tdp/groups/${groupPayload.data.groupId}?sandboxId=${encodeURIComponent(sandboxId)}`, { method: 'DELETE' });
        const deleteGroupPayload = await deleteGroupResponse.json();
    ...
    it('disables and deletes group policy materialization from terminal snapshot', async () => {
        ...
        const snapshotBeforePayload = await snapshotBefore.json();
        expect(snapshotBeforePayload.data.some(item => item.topic === 'config.delta'
            && item.scopeType === 'GROUP'
            && item.itemKey === 'config.disable.policy')).toBe(true);
        ...
        const snapshotAfterDisablePayload = await snapshotAfterDisable.json();
        expect(snapshotAfterDisablePayload.data.some(item => item.topic === 'config.delta'
            && item.scopeType === 'GROUP'
            && item.itemKey === 'config.disable.policy')).toBe(false);
        const deletePolicyResponse = await fetch(`${server.getHttpBaseUrl()}/api/v1/admin/tdp/policies/${policyPayload.data.policyId}?sandboxId=${encodeURIComponent(sandboxId)}`, { method: 'DELETE' });
    ...
    it('builds terminal decision trace with GROUP winner overriding STORE', async () => {
        ...
        expect(policyResponse.status).toBe(201);
        const traceResponse = await fetch(`${server.getHttpBaseUrl()}/api/v1/admin/tdp/terminals/${terminalId}/decision-trace?sandboxId=${encodeURIComponent(sandboxId)}`);
    ...
    it('previews selector group matches and exposes grouped stats', async () => {
        ...
        expect(recomputeResponse.status).toBe(200);
        const statsResponse = await fetch(`${server.getHttpBaseUrl()}/api/v1/admin/tdp/groups/${groupPayload.data.groupId}/stats?sandboxId=${encodeURIComponent(sandboxId)}`);
    ...
    it('updates selector group and recomputes memberships', async () => {
        ...
        const membershipsResponse = await fetch(`${server.getHttpBaseUrl()}/api/v1/admin/tdp/terminals/${activationPayload.data.terminalId}/memberships?sandboxId=${encodeURIComponent(sandboxId)}`);
    ...
    it('returns policy center overview summary', async () => {
        ...
        const preparePayload = await prepareResponse.json();
        const sandboxId = preparePayload.data.sandboxId;
        const overviewResponse = await fetch(`${server.getHttpBaseUrl()}/api/v1/admin/tdp/policy-center/overview?sandboxId=${encodeURIComponent(sandboxId)}`);
    ...
    it('supports recompute by project scope and group memberships query', async () => {
        ...
        const membershipsResponse = await fetch(`${server.getHttpBaseUrl()}/api/v1/admin/tdp/groups/${groupPayload.data.groupId}/memberships?sandboxId=${encodeURIComponent(sandboxId)}`);
        const membershipsPayload = await membershipsResponse.json();
        expect(membershipsResponse.status).toBe(200);
    ...
    it('returns policy detail by id and group recompute endpoint', async () => {
        ...
        const createPolicyPayload = await createPolicyResponse.json();
        expect(createPolicyResponse.status).toBe(201);
        const detailResponse = await fetch(`${server.getHttpBaseUrl()}/api/v1/admin/tdp/policies/${createPolicyPayload.data.policyId}?sandboxId=${encodeURIComponent(sandboxId)}`);
