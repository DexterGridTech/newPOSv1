...
import { hfs, log, readXmlLike, requireAddon, setLoggerMode, transformProps, writeAndroidAssets, writeGenericAssets, writeIOSAssets, writeWebAssets, writeXmlLike } from "./utils.js";
...
const getAndroidOutputPath = ({
  ...
  log.warn(`No ${path.relative(cwd, androidOutputPath)} directory found. Skipping Android assets generation…`);
};
const getIOSOutputPath = () => {
  ...
  log.warn(`No ${path.relative(cwd, iosOutputPath)} directory found. Skipping iOS assets generation…`);
};
const getHtmlTemplatePath = ({
  ...
  log.warn(`No ${path.relative(cwd, htmlTemplatePath)} found. Skipping HTML + CSS generation…`);
};
