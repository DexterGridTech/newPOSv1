...
import { hfs, log, PACKAGE_NAME, requireAddon, setLoggerMode, transformProps, writeAndroidAssets, writeGenericAssets, writeIOSAssets, writeWebAssets } from "./utils.js";
...
export const withBootSplash = Expo.createRunOncePlugin((expoConfig, rawProps) => {
  const {
    platforms = [],
    sdkVersion = "0.1.0"
  } = expoConfig;
  ...
  if ("licenseKey" in rawProps) {
    log.warn("Passing licenseKey directly in plugin config is deprecated as it could be leaked by expo-constants. Use the BOOTSPLASH_LICENSE_KEY environment variable instead.");
  }
  ...
  return Expo.withPlugins(expoConfig, plugins.map(plugin => [plugin, rawPropsWithEnvVariable]));
}, PACKAGE_NAME);
