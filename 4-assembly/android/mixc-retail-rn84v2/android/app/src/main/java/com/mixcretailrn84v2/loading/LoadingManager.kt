...
object LoadingManager {
    ...
    private enum class CoverType { NONE, SPLASH_SCREEN, LAUNCH_ACTIVITY, LOADING_ACTIVITY }
    ...
    /** 首次启动时用 LoadingActivity 展示自定义启动页 */
    fun showLaunchActivity(activity: Activity) {
        mainHandler.post {
            hideRequested.set(false)
            currentCover = CoverType.LAUNCH_ACTIVITY
            activity.startActivity(Intent(activity, LoadingActivity::class.java).apply {
                putExtra(LoadingActivity.EXTRA_MODE, LoadingActivity.MODE_LAUNCH)
            })
        }
    }
    ...
    fun showLoadingActivity(activity: Activity) {
        mainHandler.post {
            ...
            activity.startActivity(Intent(activity, LoadingActivity::class.java).apply {
                putExtra(LoadingActivity.EXTRA_MODE, LoadingActivity.MODE_LOADING)
            })
        }
    }
    ...
    fun hideLoading() {
        when (currentCover) {
            ...
            CoverType.LAUNCH_ACTIVITY, CoverType.LOADING_ACTIVITY -> {
                mainHandler.post {
