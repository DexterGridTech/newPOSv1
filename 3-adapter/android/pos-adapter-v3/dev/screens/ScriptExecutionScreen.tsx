...
const PRESETS = [
    {label: '基础运算', script: 'return params.a + params.b', params: '{"a": 10, "b": 20}', globals: '{}'},
    {label: '全局变量', script: 'return "Hello, " + name + "!"', params: '{}', globals: '{"name": "World"}'},
    {label: '原生函数', script: 'return greet(params.name)', params: '{"name": "POS"}', globals: '{}'},
]
...
export default function ScriptExecutionScreen() {
    ...
    const handleRun = useCallback(async () => {
        ...
        try {
            ...
            try { g = JSON.parse(globals) } catch {}
            const result = await scriptExecutionAdapter.executeScript({
                script, params: p, globals: g,
                nativeFunctions: {greet: (name: string) => `Hi, ${name}!`},
    ...
    const handleGetStats = useCallback(async () => {
        setStats(await NativeScriptsTurboModule.getStats())
    }, [])
                    ...
                    <View style={s.row}>
                        {PRESETS.map(p => (
                            <TouchableOpacity key={p.label} style={s.chip} onPress={() => {
                                setScript(p.script); setParams(p.params); setGlobals(p.globals)
                            }}>
