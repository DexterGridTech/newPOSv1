...
object HotUpdateManifestCodec {
  fun decodePackage(input: String): HotUpdateManifestPackage? {
    val packageBody = Regex("""\"package\"\s*:\s*\{(.*?)\}""", setOf(RegexOption.DOT_MATCHES_ALL))
      .find(input)
      ?.groupValues
      ?.getOrNull(1)
      ?: return null
    val entry = HotUpdateMarkerCodec.findString(packageBody, "entry") ?: return null
    val sha256 = HotUpdateMarkerCodec.findString(packageBody, "sha256") ?: return null
    return HotUpdateManifestPackage(
      entry = entry,
      sha256 = sha256,
    )
  }
