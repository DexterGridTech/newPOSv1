...
class LocalWebServer(
    ...
    fun start() {
        try {
            ...
            Log.e(TAG, "Port ${config.port} already in use", e)
            throw IllegalStateException("Failed to start server: port in use", e)
        }
            ...
            while (isRunning) {
                try {
                    ...
                    try {
                        ...
                        Log.w(TAG, "Executor queue full, closing client")
    ...
    private inner class RetryQueue(
        ...
        fun enqueue(raw: String) {
            ...
            if (messages.size >= maxSize) {
                Log.w(TAG, "RetryQueue full for $masterDeviceId, dropping oldest message")
                messages.removeAt(0)
            }
