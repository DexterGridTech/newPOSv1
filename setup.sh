...
check_node() {
    ...
    if ! command_exists node; then
        print_error "未检测到 Node.js，请先安装 Node.js $REQUIRED_NODE_VERSION 或更高版本"
    ...
    if [ "$NODE_MAJOR" -lt "$REQUIRED_NODE_MAJOR" ]; then
        print_error "Node.js 版本过低 (当前: v$NODE_VERSION)，需要 >= $REQUIRED_NODE_VERSION"
        exit 1
    fi
...
print_next_steps() {
    ...
    echo ""
    echo "  5. 真机调试前执行端口转发:"
    echo "     ${YELLOW}yarn android:port-forward${NC}"
    echo ""
    echo "  6. 查看所有可用命令:"
    ...
    echo -e "${BLUE}更多信息请查看 package.json、AGENTS.md 与 spec/ 下的方法论文档${NC}"
    echo ""
}
...
main() {
    ...
    echo ""
    echo -e "${GREEN}╔════════════════════════════════════════╗${NC}"
    echo -e "${GREEN}║   Next 项目自动化安装脚本           ║${NC}"
    echo -e "${GREEN}╚════════════════════════════════════════╝${NC}"
    echo ""
