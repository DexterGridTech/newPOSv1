...
const waitForPageReady = async (session) => {
    ...
    while (Date.now() < deadline) {
        const ready = await evalInBrowser(session, `Boolean(document.body.innerText.includes('Terminal Console Test Expo'))`)
        if (ready) {
            return
        }
        await delay(500)
    }
    throw new Error(`Timed out waiting for terminal-console Expo page in ${session}`)
}
...
const readState = async (session) => evalInBrowser(session, `JSON.stringify({
    ready: Boolean(document.querySelector('[data-testid="ui-base-terminal-console-expo:ready"]')),
    activationCode: document.querySelector('[data-testid="ui-base-terminal-console-expo:activation-code"]')?.textContent ?? null,
    activationStatus: document.querySelector('[data-testid="ui-base-terminal-console-expo:activation-status"]')?.textContent ?? null,
    ...
    summaryDescription: document.querySelector('[data-testid="ui-base-terminal-summary:description"]')?.textContent ?? null,
