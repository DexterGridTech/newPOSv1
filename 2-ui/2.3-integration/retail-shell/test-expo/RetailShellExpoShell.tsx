                    ...
                    createHttpRuntime(context) {
                        return createHttpRuntime({
                            logger: context.platformPorts.logger.scope({
                                moduleName: 'ui.integration.retail-shell.test-expo',
...
export const RetailShellExpoShell: React.FC = () => {
                ...
                await nextHarness.runtime.dispatchCommand(createCommand(
                    tcpControlV2CommandDefinitions.bootstrapTcpControl,
                    {
                        deviceInfo: {
                            ...
                            model: 'Retail Shell Expo Mock POS',
                        },
                    },
                ))
    ...
    if (bootError) {
        return (
            <View testID="ui-integration-retail-shell-expo:error">
                <Text>{bootError}</Text>
            </View>
        )
    }
    ...
    if (!harness) {
        return (
            <View testID="ui-integration-retail-shell-expo:loading">
                <Text>Retail Shell Expo Loading</Text>
            </View>
        )
    }
        ...
        <Provider store={harness.store}>
            <UiRuntimeProvider runtime={harness.runtime}>
                <View style={{flex: 1, backgroundColor: '#eef4fa'}}>
                    ...
                    <RetailShellExpoStateCard activationCode={activationCode} />
                </View>
            </UiRuntimeProvider>
        </Provider>
    )
...
const RetailShellExpoStateCard: React.FC<{
    ...
    return (
        <View
            testID="ui-integration-retail-shell-expo:ready"
            style={{
                position: 'absolute',
                right: 16,
                bottom: 16,
        ...
        >
            <Text style={{fontSize: 24, fontWeight: '800', color: '#0f172a'}}>
                Retail Shell Test Expo
            </Text>
            <Text selectable testID="ui-integration-retail-shell-expo:activation-code">
                {activationCode}
            </Text>
            <Text selectable testID="ui-integration-retail-shell-expo:activation-status">
                {identity.activationStatus}
            </Text>
            <Text selectable testID="ui-integration-retail-shell-expo:terminal-id">
                {identity.terminalId ?? 'unactivated'}
            </Text>
            <Text selectable testID="ui-integration-retail-shell-expo:admin-password">
                {adminPassword}
            </Text>
        </View>
    )
