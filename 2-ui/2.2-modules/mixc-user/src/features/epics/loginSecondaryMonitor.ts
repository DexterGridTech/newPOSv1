...
import {mpLoginScreenPart} from '../../ui/screens/MPLoginScreen';
import {spLoginScreenPart} from '../../ui/screens/SPLoginScreen';
 ...
 */
export const loginSecondaryMonitorEpic: Epic<PayloadAction, PayloadAction, RootState> = (action$, state$) => {
    return state$.pipe(
        ...
        filter((containerValue) => {
            return containerValue?.partKey === mpLoginScreenPart.partKey;
        }),

        // 副屏显示（副作用）
        tap(() => {
            kernelCoreNavigationCommands.navigateTo({target: spLoginScreenPart}).executeInternally();
        }),
