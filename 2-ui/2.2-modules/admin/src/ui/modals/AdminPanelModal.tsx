...
export const AdminPanelModal: React.FC<ModalScreen<AdminPanelModalProps>> = React.memo((model) => {
    ...
    // 获取当前选中的子 ScreenPart
    const currentChild = useChildScreenPart(uiAdminVariables.systemAdminPanel);

    // 动态获取所有注册到 systemAdminPanel 的 ScreenPart
    const menuItems = useMemo(() => {
        return getScreenPartsByContainerKey(uiAdminVariables.systemAdminPanel.key);
    }, []);
    ...
    useLifecycle({
        ...
        onClearance: useCallback(() => {
            kernelCoreNavigationCommands.clearUiVariables([uiAdminVariables.systemAdminPanel.key]).executeInternally();
        }, [model.id]),
    });
