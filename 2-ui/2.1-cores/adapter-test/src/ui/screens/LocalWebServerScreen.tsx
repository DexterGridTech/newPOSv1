...
import {localWebServer} from '@impos2/kernel-core-interconnection'
...
export default function LocalWebServerScreen() {
    ...
    const handleStart = () => run('start', async () => {
        const addresses = await localWebServer.startLocalWebServer({port: 8888})
                ...
                <View style={[s.btnRow, isSmall && {flexDirection: 'column'}]}>
                    {[
                        ...
                        <TouchableOpacity key={b.key} style={[s.btn, {backgroundColor: b.color}]}
                            onPress={b.fn} disabled={loading !== null}>
