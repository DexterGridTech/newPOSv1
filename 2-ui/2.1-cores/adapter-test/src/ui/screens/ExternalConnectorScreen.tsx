...
function RRPanel({addLog, sharedTarget, onTargetSelect}: {
    ...
    const handleCall = useCallback(async () => {
        ...
        try {
            ...
            const res = await externalConnector.call(buildChannel(), action, p, parseInt(timeout) || 5000)
            addLog({
    ...
    const handleCheck = useCallback(async () => {
        try {
            const r = await externalConnector.isAvailable(buildChannel())
            setAvailable(r)
    ...
    const handleGetTargets = useCallback(async () => {
        try {
            const list = await externalConnector.getAvailableTargets(channelType)
            setTargets(list)
...
function StreamPanel({addLog, sharedTarget}: {
    ...
    const applyPreset = useCallback((t: ChannelType) => {
        setChannelType(t)
        setTarget(STREAM_PRESETS[t].target)
    }, [])
    ...
    const handleSubscribe = useCallback(async () => {
        ...
        setEventCount(0)
        try {
            const id = await externalConnector.subscribe(
                buildChannel(),
                (event: ConnectorEvent) => {
                    setEventCount(c => c + 1)
                    addLog({
                        mode: 'stream', channelType, success: true,
                ...
                (error: ConnectorEvent) => {
                    addLog({
                        mode: 'stream', channelType, success: false,
                        label: `[${channelType}] 错误`,
                        body: error.raw ?? '硬件异常',
                    })
                },
            )
            ...
            addLog({mode: 'stream', channelType, success: true,
                label: `订阅成功`, body: `channelId: ${id}`})
        } catch (e: any) {
            setSubscribing(false)
            addLog({mode: 'stream', channelType, success: false,
                label: '订阅失败', body: e?.message ?? String(e)})
        }
    ...
    const handleUnsubscribe = useCallback(async () => {
        if (!channelId) return
        try {
            await externalConnector.unsubscribe(channelId)
            addLog({mode: 'stream', channelType, success: true,
                label: '已取消订阅', body: `channelId: ${channelId}`})
        } catch (e: any) {
            addLog({mode: 'stream', channelType, success: false,
                label: '取消订阅失败', body: e?.message ?? String(e)})
        } finally {
            setChannelId(null)
            setSubscribing(false)
        }
    ...
    const handleCheck = useCallback(async () => {
        try {
            ...
            addLog({mode: 'stream', channelType, success: false,
                label: 'isAvailable ERROR', body: e?.message ?? String(e)})
        }
    }, [channelType, target, options])

    useEffect(() => () => {
        if (channelId) {
            externalConnector.unsubscribe(channelId).catch(() => {})
        }
    }, [channelId])

    // 仅支持 stream 的通道类型
    const STREAM_TYPES: ChannelType[] = ['USB', 'SERIAL', 'BLUETOOTH']
    ...
    return (
        <ScrollView contentContainerStyle={{paddingBottom: 32}} keyboardShouldPersistTaps="handled">
            ...
            <View style={[s.statusBar, {borderColor: subscribing ? TYPE_COLOR[channelType] : C.border}]}>
                ...
                {channelId && (
                    <Text style={s.channelIdText} numberOfLines={1}>{channelId.slice(0, 16)}…</Text>
                )}
            </View>

            <View style={s.btnRow}>
                <TouchableOpacity
                    style={[s.btn, {backgroundColor: subscribing ? C.textMuted : TYPE_COLOR[channelType]}]}
                    onPress={handleSubscribe} disabled={subscribing}>
                    <Text style={s.btnText}>SUBSCRIBE</Text>
                </TouchableOpacity>
                <TouchableOpacity
                    style={[s.btn, {backgroundColor: subscribing ? C.danger : C.textMuted}]}
...
function PassivePanel({addLog, sharedTarget}: {
    ...
    const handleListen = useCallback(() => {
        ...
        setListening(true)
        setEventCount(0)
        const remove = externalConnector.on('connector.passive', (event: ConnectorEvent) => {
            // 若填了 target 则过滤，否则接收全部
    ...
    return (
        <ScrollView contentContainerStyle={{paddingBottom: 32}} keyboardShouldPersistTaps="handled">
            ...
            <View style={s.btnRow}>
                <TouchableOpacity
                    style={[s.btn, {backgroundColor: listening ? C.textMuted : C.accent}]}
                    onPress={handleListen} disabled={listening}>
                    <Text style={s.btnText}>START LISTEN</Text>
                </TouchableOpacity>
                <TouchableOpacity
                    style={[s.btn, {backgroundColor: listening ? C.danger : C.textMuted}]}
        ...
        <TouchableOpacity onPress={() => setExpanded(e => !e)} activeOpacity={0.8}>
            <View style={[s.logRow, {borderLeftColor: color, borderLeftWidth: 3}]}>
                <View style={s.logMeta}>
                    ...
                    <Text style={[s.logTypeBadge, {color: TYPE_COLOR[item.channelType as ChannelType] ?? C.textMuted,
