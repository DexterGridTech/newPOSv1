...
import {stateStorage} from '@impos2/kernel-core-base'
import {C} from '../theme'
...
export default function StateStorageScreen() {
    ...
    const handleGet = useCallback(async () => {
        if (!key.trim()) return
        try {
            const val = await stateStorage.getItem(key.trim())
    ...
    const handleSet = useCallback(async () => {
        ...
        try {
            ...
            await stateStorage.setItem(key.trim(), parsed)
            setResult('OK')
    ...
    const handleRemove = useCallback(async () => {
        if (!key.trim()) return
        try {
            await stateStorage.removeItem(key.trim())
            setResult('Removed')
