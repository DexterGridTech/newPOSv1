...
import {mpActivateDeviceScreenPart} from '../../ui/screens/MPActivateDeviceScreen';
 ...
 */
export const activateDeviceSecondaryMonitorEpic: Epic<PayloadAction, PayloadAction, RootState> = (action$, state$) => {
    return state$.pipe(
        ...
        filter((containerValue) => {
            return containerValue?.partKey === mpActivateDeviceScreenPart.partKey;
        }),
        // 副屏显示（副作用）
        tap((containerValue) => {
            kernelCoreNavigationCommands.navigateTo({target: mpActivateDeviceScreenPart}).executeInternally()
        }),
        // 不发出任何 action
        ignoreElements()
    );
