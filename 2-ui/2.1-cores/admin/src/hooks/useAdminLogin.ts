...
import {uiCoreAdminVariables} from "../ui/variables";
...
export const useAdminLogin = ({modalId, onComplete}: UseAdminLoginOptions) => {
    ...
    const {value: password, setValue: setPassword} = useEditableUiVariable(uiCoreAdminVariables.adminPassword);
    ...
    const handleSubmit = useCallback(() => {
        ...
        const id = shortId();
        setRequestId(id);
        uiCoreAdminCommands.adminLogin({adminPassword: password}).execute(id);
    }, [loginStatus?.status, password]);

    const cleanup = useCallback(() => {
        kernelCoreNavigationCommands.clearUiVariables([uiCoreAdminVariables.adminPassword.key]).executeInternally();
    }, []);
