...
function checkNode() {
  ...
  const requiredNodeVersion = '22.11.0';
  const requiredNodeMajor = parseInt(requiredNodeVersion.split('.')[0], 10);
  ...
  if (nodeMajor < requiredNodeMajor) {
    printError(`Node.js 版本过低: ${nodeVersion} (需要 >= ${requiredNodeVersion})`);
    return false;
  }
...
function checkAndroidSDK() {
  ...
  if (!androidHome) {
    ...
    console.log('  export PATH=$PATH:$ANDROID_HOME/emulator');
    console.log('  export PATH=$PATH:$ANDROID_HOME/cmdline-tools/latest/bin');
    return false;
  }
