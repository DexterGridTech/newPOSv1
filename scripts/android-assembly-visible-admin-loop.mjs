...
function assertSecondaryAutomationAlive(label) {
  ...
  const tree = unwrapJsonRpcResult(treeResponse, 'secondary ui.getTree')
  if (!Array.isArray(tree) || tree.length === 0) {
    fail(`${label}: secondary automation tree is empty`, JSON.stringify({hello, treeResponse}, null, 2))
  }
  return {
    hello,
    treeNodeCount: tree.length,
    activity: assertSecondaryActivityVisible(label),
  }
}
...
async function main() {
  ...
  assertSecondaryAlive('before flow')
