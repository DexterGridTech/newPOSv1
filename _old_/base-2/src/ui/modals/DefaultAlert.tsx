...
import {CommandRegistry, ScreenMode, logger, LOG_TAGS} from "_old_/base";
...
export const DefaultAlert: React.FC<ModalScreen<AlertInfo>> = React.memo((model) => {
     ...
     */
    if (!model.props) {
        logger.warn([moduleName, LOG_TAGS.UI, 'DefaultAlert'], 'Missing props', { modelId: model.id });
        return null;
    }

    if (!model.props.title && !model.props.message) {
        logger.warn([moduleName, LOG_TAGS.UI, 'DefaultAlert'], 'Missing title and message', { modelId: model.id });
        return null;
    }
