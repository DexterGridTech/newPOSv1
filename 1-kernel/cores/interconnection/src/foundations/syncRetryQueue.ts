...
export class SyncRetryQueue {
    ...
    enqueue(stateKey: string, changes: Record<string, any>, targetDevice: string): void {
        // 合并同一 stateKey + targetDevice 的变更
        const existing = this.queue.find(
            item => item.stateKey === stateKey && item.targetDevice === targetDevice
        )
        if (existing) {
    ...
    private async processQueue(): Promise<void> {
        ...
        for (const item of items) {
            try {
                await syncStateToSlave(item.stateKey, item.changes, item.targetDevice)
