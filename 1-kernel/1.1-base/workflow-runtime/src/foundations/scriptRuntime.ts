...
const executeLocalScript = async <T = unknown>(input: {
    ...
    return await new Promise<T>((resolve, reject) => {
        ...
        const timer = setTimeout(() => {
            ...
            settled = true
            reject(createAppError(workflowRuntimeErrorDefinitions.workflowScriptFailed, {
                details: {
                    reason: 'timeout',
                    timeoutMs,
...
const executeScript = async <T = unknown>(input: {
    ...
    if (executor) {
        try {
            ...
            throw createAppError(workflowRuntimeErrorDefinitions.workflowScriptFailed, {
                details: {
                    reason: 'port-failed',
