...
describe('tcp-control-runtime', () => {
    it('activates terminal, refreshes credential, reports task result, and persists only recovery state', async () => {
        ...
        const activationResult = await runtime.execute({
        ...
        expect(
            stateStorage.saved.has('kernel-runtime:node_tcp_test:app-state:kernel.base.tcp-control-runtime.runtime:bootstrapped'),
        ).toBe(false)
