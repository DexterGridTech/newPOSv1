...
export const createTerminalTopologyIntegrationHarnessV2 = async (input?: {
    ...
    const topologyHost = await createTopologyRuntimeV2LiveHarness({
        profileName: 'dual-topology.ws.topology-runtime-v2.terminal-integration',
    })

    const terminalSocketRuntime = createSocketRuntime({
        logger: createTestLogger('dual-topology.ws.topology-runtime-v2.terminal-socket'),
    ...
    const masterRuntime = topologyHost.createMasterRuntime([
        ...((input?.includeDefaultBridgeModule ?? true) ? [createTerminalBridgeModuleV2()] : []),
        ...(input?.masterModules ?? []),
        createTcpControlRuntimeModuleV2({
            assembly: {
                createHttpRuntime() {
                    return createHttpRuntimeForTerminal('dual-topology.ws.topology-runtime-v2.tcp-http')
                },
            },
        }),
        createTdpSyncRuntimeModuleV2({
            ...
            assembly: {
                createHttpRuntime() {
                    return createHttpRuntimeForTerminal('dual-topology.ws.topology-runtime-v2.tdp-http')
                },
    ...
    return {
        ...
        async configureTopologyPair() {
            ...
            await slaveRuntime.dispatchCommand(createCommand(topologyRuntimeV2CommandDefinitions.setMasterInfo, {
                masterInfo: {
                    ...
                    serverAddress: [{address: topologyHost.addressInfo.wsUrl}],
        ...
        async disconnect() {
            ...
            topologyHost.masterSocketRuntime.disconnect('dual-topology.ws.topology-runtime-v2.terminal-integration', 'test-complete')
            topologyHost.slaveSocketRuntime.disconnect('dual-topology.ws.topology-runtime-v2.terminal-integration', 'test-complete')
