...
import {selectTdpCommandInboxState} from '@impos2/kernel-base-tdp-sync-runtime'
import {
    createTerminalTopologyBridgeLiveHarness,
} from '../helpers/terminalTopologyBridgeHarness'
import {
    createTerminalRemoteCommandPanelModule,
    selectTerminalRemoteCommandPanelState,
} from '../helpers/remoteCommandReadModelModule'
...
describe('topology-client-runtime live remote command panel', () => {
    it('builds a business-style remote command read model from TDP command inbox and syncs it to slave through real topology state sync', async () => {
        const remoteCommandPanelModule = createTerminalRemoteCommandPanelModule()
        ...
        try {
            ...
            await harness.activateAndConnectTerminal({
                ...
                deviceId: 'device-live-remote-command-panel-001',
            })
            ...
            const releaseResponse = await fetch(`${harness.terminalBaseUrl}/api/v1/admin/tasks/releases`, {
                ...
                body: JSON.stringify({
                    title: 'topology-client-runtime-live-remote-command-panel',
                    taskType: 'REMOTE_CONTROL',
                    sourceType: 'COMMAND',
                    sourceId: 'topology-client-runtime-live-remote-command-panel',
                    priority: 1,
                    ...
                    payload: {
                        ...
                        action: 'SYNC_ORDER',
                        businessKey: 'order-20260411-remote-panel-001',
                    },
                }),
            })
            ...
            await waitFor(() => {
                const inbox = selectTdpCommandInboxState(harness.masterRuntime.getState())
            ...
            const reportResult = await harness.reportTaskResult({
                ...
                status: 'COMPLETED',
                result: {
                    handledBy: 'topology-client-runtime-live-remote-command-panel',
