...
describe('topology-client-runtime live terminal bridge', () => {
    it('mirrors terminal projection data from master runtime to slave runtime through real topology state sync', async () => {
        ...
        try {
            ...
            await harness.activateAndConnectTerminal({
                ...
                deviceId: 'device-live-terminal-topology-bridge-001',
            })

            await harness.upsertProjection({
                topicKey: 'config.delta',
                payload: {
                    configVersion: 'terminal-topology-bridge-001',
                    featureFlag: true,
                },
            })
            ...
            const projectionState = selectTdpProjectionState(harness.masterRuntime.getState())
            const itemKey = Object.keys(projectionState?.byTopic['config.delta'] ?? {})[0]
            if (!itemKey) {
