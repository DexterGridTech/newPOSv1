import { LOG_TAGS, logger, PERSIST_KEY } from "@impos2/kernel-core-base";
        ...
        if (!prevStartToSync && nextStartToSync) {
            ...
            statesNeedToSync.forEach(stateKey => {
                ...
                if (state) {
                    ...
                    cachedPropertyRefs[stateKey] = Object.fromEntries(Object.entries(state).filter(([k]) => k !== PERSIST_KEY));
                }
            });
        }
            ...
            const detectedSliceRefs = {};
            statesNeedToSync.forEach(stateKey => {
                const currentSlice = nextState[stateKey];
                // 优化1：slice 引用相同 = 没变化，直接跳过
                if (currentSlice === cachedSliceRefs[stateKey])
                    return;
                ...
                for (const key of Object.keys(current)) {
                    ...
                    if (prop && typeof prop === 'object' && prop.updatedAt) {
                        if (typeof prop.updatedAt !== 'number' || isNaN(prop.updatedAt)) {
                            logger.warn([moduleName, LOG_TAGS.System, "stateSyncMiddleware"], `属性 ${String(stateKey)}.${key} 的 updatedAt 不是有效数字:`, prop.updatedAt);
                            continue;
                        }
            ...
            if (Object.keys(changesToSync).length > 0) {
                const syncPromises = Object.keys(changesToSync).map(stateKey => syncStateToRemote(stateKey, changesToSync[stateKey]));
