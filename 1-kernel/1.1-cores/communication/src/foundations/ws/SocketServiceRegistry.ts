...
export class SocketServiceRegistry {
  ...
  getModule<TServices>(moduleName: string): TServices {
    if (!this.modules.has(moduleName)) {
      throw new CommunicationError('NOT_IMPLEMENTED', `Socket service module 未注册: ${moduleName}`)
    }
...
export const socketServiceRegistry = new SocketServiceRegistry()
