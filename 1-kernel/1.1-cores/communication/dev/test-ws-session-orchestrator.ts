...
export async function testWsSessionOrchestrator() {
  ...
  serverResolver.registerServer({
    serverName: 'communicationWsSession',
  ...
  const bootstrapEndpoint: HttpEndpointDefinition<never, never, BootstrapRequest, BootstrapResponse> = defineHttpEndpoint({
    name: 'test.ws.session.bootstrap',
    serverName: 'communicationHttp',
    method: 'POST',
    pathTemplate: '/ws/bootstrap-session',
    request: {
  ...
  const profile = defineSocketProfile<{deviceId: string; token: string}, Record<string, string>, {type: string; payload?: unknown}, {type: string; payload?: unknown}>({
    name: 'test.ws.session.profile',
    serverName: 'communicationWsSession',
    pathTemplate: '/ws/session-connect',
    handshake: {
  ...
  socketClient.send({type: 'PING', payload: 'hello-session'})
  await new Promise(resolve => setTimeout(resolve, 120))
  socketClient.disconnect('done')

  if (result.descriptor.query?.deviceId !== 'DEVICE-SESSION-1' || !result.descriptor.query?.token) {
    throw new Error(`bootstrap descriptor 异常: ${JSON.stringify(result.descriptor)}`)
  }

  if (!events.includes('CONNECTED') || !events.includes('SESSION_READY') || !events.includes('SESSION_ECHO')) {
    throw new Error(`session orchestrator 事件异常: ${JSON.stringify(events)}`)
  }

  return {name: 'testWsSessionOrchestrator', passed: true}
}
