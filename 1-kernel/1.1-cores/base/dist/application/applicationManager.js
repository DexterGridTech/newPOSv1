...
import { ActorSystem, ApiManager, registerModuleCommands, registerModuleErrorMessages, registerModuleSystemParameter, screenPartRegisters, setStateStoragePrefix, stateStorage, taskDefinitionRegisters } from "../foundations";
...
export class ApplicationManager {
    ...
    async generateStore(config) {
        ...
        let selectedServerSpace = await storeEntry.getSelectServerSpace();
        ...
        const middlewares = [epicMiddleware, ...moduleMiddlewares.map(m => m.middleware)];
        const middlewareNames = ['epicMiddleware', ...moduleMiddlewares.map(m => `${m.name}(p:${m.priority})`)];
        ...
        if (config.reactotronEnhancer) {
            storeOptions.enhancers = (getDefaultEnhancers) => getDefaultEnhancers().concat(config.reactotronEnhancer);
        }
    ...
    initApiServerAddress(serverSpace, selectedSpace) {
