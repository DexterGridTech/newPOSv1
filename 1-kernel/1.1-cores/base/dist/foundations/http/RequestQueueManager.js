/**
 * 请求队列管理器
 * 负责请求队列管理、并发控制和限流
 */
import { DEFAULT_CONFIG, PERFORMANCE_CONFIG } from '../../types/shared/http';
/**
 * 请求队列类
 */
export class RequestQueue {
    maxConcurrent;
    rateLimitWindow;
    rateLimitMax;
    queue = [];
    activeCount = 0;
    requestTimestamps = [];
    constructor(maxConcurrent = DEFAULT_CONFIG.MAX_CONCURRENT_REQUESTS, rateLimitWindow = DEFAULT_CONFIG.RATE_LIMIT_WINDOW, rateLimitMax = DEFAULT_CONFIG.RATE_LIMIT_MAX_REQUESTS) {
        this.maxConcurrent = maxConcurrent;
        this.rateLimitWindow = rateLimitWindow;
        this.rateLimitMax = rateLimitMax;
    }
    async enqueue(fn) {
        // 速率限制检查
        this.checkRateLimit();
        // 并发控制
        if (this.activeCount >= this.maxConcurrent) {
            await new Promise((resolve, reject) => this.queue.push({ resolve, reject }));
        }
        this.activeCount++;
        this.recordRequest();
        try {
            return await fn();
        }
        finally {
            this.activeCount--;
            const next = this.queue.shift();
            if (next)
                next.resolve();
        }
    }
    checkRateLimit() {
        const currentTime = Date.now();
        this.requestTimestamps = this.requestTimestamps.filter(timestamp => currentTime - timestamp < this.rateLimitWindow);
        if (this.requestTimestamps.length >= this.rateLimitMax) {
            throw new Error('Rate limit exceeded');
        }
    }
    recordRequest() {
        const currentTime = Date.now();
        this.requestTimestamps.push(currentTime);
        // 使用常量替代魔法数字
        if (this.requestTimestamps.length % PERFORMANCE_CONFIG.CLEANUP_INTERVAL === 0 ||
            this.requestTimestamps.length > PERFORMANCE_CONFIG.MAX_TIMESTAMPS) {
            const cleanupTime = Date.now();
            this.requestTimestamps = this.requestTimestamps.filter(timestamp => cleanupTime - timestamp < this.rateLimitWindow);
        }
    }
    getStats() {
        return {
            activeCount: this.activeCount,
            queueLength: this.queue.length,
            recentRequestCount: this.requestTimestamps.length
        };
    }
    /**
     * 清理队列和时间戳,用于内存管理
     * 注意：这会拒绝所有等待中的请求
     */
    cleanup() {
        // 拒绝所有等待中的请求
        const queuedRequests = this.queue;
        this.queue = [];
        // 明确拒绝所有等待的请求，避免内存泄漏
        queuedRequests.forEach(({ reject }) => {
            reject(new Error('Request queue cleanup: all pending requests cancelled'));
        });
        // 清空时间戳
        this.requestTimestamps = [];
    }
    /**
     * 获取当前队列大小
     */
    getQueueSize() {
        return this.queue.length;
    }
}
