...
export function executeScriptAsObservable<T = any>(
    ...
    return defer(() =>
        ...
        map(data => {
            ...
            if (data !== null && typeof data === 'object' && '$error' in (data as any)) {
                return {
                    success: false,
                    error: { code: 'SCRIPT_EXEC_ERROR', message: (data as any).$error, retryable: false }
                } as ScriptResult<T>;
            }
            return {success: true, data} as ScriptResult<T>;
        }),
        catchError(err => {
