import { getScreenMode, ScreenMode } from "@impos2/kernel-core-base";
...
const screenRegistryMap = {
    [ScreenMode.MOBILE]: new Map(),
    [ScreenMode.DESKTOP]: new Map()
};
/**
 * 注册 ScreenPart
 * @param registration ScreenPart 完整注册信息
 */
export function registerScreenPart(registration) {
...
export function getScreenPartComponentType(key) {
 ...
 */
export function getScreenPartReadyToEnter(key) {
    const screenMode = getScreenMode();
    return screenRegistryMap[screenMode].get(key)?.readyToEnter;
}
export function getFirstReadyScreenPartByContainerKey(containerKey, fromIndex) {
    const screenMode = getScreenMode();
 ...
 */
export function getScreenPartsByContainerKey(containerKey) {
    const screenMode = getScreenMode();
    const instanceMode = getInstanceMode();
    const workspace = getWorkspace();
...
export const createModelScreen = (screenPart, id, props) => {
    return { ...screenPart, id, props };
};
export const defaultAlertPartKey = "alert";
export const createAlert = (id, props) => {
    return {
        id,
        props,
