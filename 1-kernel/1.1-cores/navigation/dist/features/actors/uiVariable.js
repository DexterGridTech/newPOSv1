...
export class UiVariableActor extends Actor {
    setUiVariables = Actor.defineCommandHandler(kernelCoreNavigationCommands.setUiVariables, async (command) => {
        dispatchWorkspaceAction(uiVariablesActions.updateUiVariables(command.payload), command);
        return {};
    });
    clearUiVariables = Actor.defineCommandHandler(kernelCoreNavigationCommands.clearUiVariables, async (command) => {
