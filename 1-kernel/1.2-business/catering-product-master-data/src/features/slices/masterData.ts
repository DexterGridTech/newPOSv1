...
const createEmptyState = (changedAt?: number): CateringProductMasterDataState => ({
    byTopic: {},
    diagnostics: [],
    ...
    sync: {
        ...
        applyEntries: (
            ...
            Object.entries(entries).forEach(([entryKey, entry]: [string, SyncValueEnvelope | undefined]) => {
                const [topic, itemKey] = entryKey.split(':', 2) as [CateringProductTopic | undefined, string | undefined]
                if (!topic || !itemKey) {
                    return
                }
