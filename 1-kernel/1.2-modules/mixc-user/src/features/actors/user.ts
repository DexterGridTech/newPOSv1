...
import {kernelMixcUserApis} from "../../supports";
import {userActions} from "../slices/user";
import {User} from "../../types";

export class UserActor extends Actor {
    loginWithPassword = Actor.defineCommandHandler(kernelMixcUserCommands.loginWithPassword,
        async (command): Promise<Record<string, any>> => {
            ...
            if(command.payload.userName==='boss'&&command.payload.password==='boss'){
                ...
                kernelMixcUserCommands.loginSuccess().executeFromParent(command)
                return {
                    user: user
                }
            }
            ...
            const result = await kernelMixcUserApis.loginWithPassword.run({request: loginWithPasswordRequest})
    ...
    sendVerifyCode = Actor.defineCommandHandler(kernelMixcUserCommands.sendVerifyCode,
        async (command): Promise<Record<string, any>> => {
            ...
            const result = await kernelMixcUserApis.sendVerifyCode.run({request: sendVerifyCodeRequest})
            if (result.code === APIResponseCode.SUCCESS) {
                return {}
            } else {
                throw new APIError(result)
    ...
    loginWithMobile = Actor.defineCommandHandler(kernelMixcUserCommands.loginWithMobile,
        async (command): Promise<Record<string, any>> => {
            ...
            const result = await kernelMixcUserApis.loginWithMobile.run({request: loginWithMobileRequest})
            if (result.code === APIResponseCode.SUCCESS) {
                storeEntry.dispatchAction(userActions.setUser(result.data?.user!))
                kernelMixcUserCommands.loginSuccess().executeFromParent(command)
                return {
    ...
    loginWithBarcode = Actor.defineCommandHandler(kernelMixcUserCommands.loginWithBarcode,
        async (command): Promise<Record<string, any>> => {
            logger.log([moduleName, LOG_TAGS.Actor, "UserActor"], 'login With Barcode')
            const loginWithBarcodeRequest: LoginWithBarcodeRequest = {
                deviceId:getDeviceId()
            }
            const result = await kernelMixcUserApis.loginWithBarcode.run({request: loginWithBarcodeRequest})
            if (result.code === APIResponseCode.SUCCESS) {
                return {
                    url: result.data?.url!
                }
    ...
    logout = Actor.defineCommandHandler(kernelMixcUserCommands.logout,
        async (command): Promise<Record<string, any>> => {
            logger.log([moduleName, LOG_TAGS.Actor, "UserActor"], 'logout')
            const logoutRequest: LogoutRequest = {
                deviceId:getDeviceId()
            }
            const result = await kernelMixcUserApis.logout.run({request: logoutRequest})
            if (result.code === APIResponseCode.SUCCESS) {
    ...
    updateUserState = Actor.defineCommandHandler(kernelMixcUserCommands.updateUserState,
        async (command): Promise<Record<string, any>> => {
