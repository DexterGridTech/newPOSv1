...
import {kernelMixcOrderPayCommands} from "../commands";
...
import {
    ...
    MainOrderBaseStatus
} from "@impos2/kernel-mixc-order-base";
...
export class PayingOrderActor extends Actor {
    addPayingOrderFromDraft = Actor.defineCommandHandler(kernelMixcOrderPayCommands.addPayingOrderFromDraft,
        async (command): Promise<Record<string, any>> => {
            ...
            const productOrders=command.payload
                ...
                .map((productOrder,index,all)=>({...productOrder,
                    productCode:generateProductOrderCode(subOrderCode,all.length,index),
                }))
