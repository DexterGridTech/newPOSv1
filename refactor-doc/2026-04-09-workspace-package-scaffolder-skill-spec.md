...
`1-kernel/1.1-base` 的 7 个基础包必须共享统一目录结构，但脚手架后续应按职责生成差异化占位文件。
...
1. pair host 语义
2. pairing/session/relay/observability/fault-injection 五个职责面
...
- 导出最小公共入口
- 保留后续补实现的位置
- 不默认把内部 `features/slices`、`features/actors`、`features/middlewares` 当成公开 API
